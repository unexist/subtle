...
void *subSharedMemoryAlloc(size_t n, size_t size) {
    void *mem = NULL;
...
void *subSharedMemoryRealloc(void *mem, size_t size) {
    /* Check result */
...
regex_t *subSharedRegexNew(char *pattern) {
    int ecode = 0;
    regex_t *preg = NULL;
    OnigErrorInfo einfo;
    ...
    /* Create onig regex */
    ecode = onig_new(&preg, (UChar *) pattern, (UChar *) (pattern + strlen(pattern)),
...
int subSharedRegexMatch(regex_t *preg, char *string) {
    ...
    return ONIG_MISMATCH != onig_match(preg, (UChar *) string, (UChar *) (string + strlen(string)),
                                       (UChar *) string, NULL, ONIG_OPTION_NONE);
} /* }}} */
...
char *subSharedPropertyGet(Display *disp, Window win, Atom type, Atom prop, unsigned long *size) {
    int format = 0;
    ...
    /* Get property */
    if (Success != XGetWindowProperty(disp, win, prop, 0L, 4096, False, type, &rtype, &format,
                                      &nitems, &bytes, &data)) {
        return NULL;
    }
...
char **subSharedPropertyGetStrings(Display *disp, Window win, Atom prop, int *nlist) {
    char **list = NULL;
    XTextProperty text;
    ...
    if ((XGetTextProperty(disp, win, &text, prop) ||
         XGetTextProperty(disp, win, &text, XA_STRING)) &&
        text.nitems)
    {
...
void subSharedPropertySetStrings(Display *disp, Window win, Atom prop, char **list, int nlist) {
    XTextProperty text;
...
void subSharedPropertyName(Display *disp, Window win, char **name, char *fallback) {
    char **list = NULL;
    XTextProperty text;
    ...
    if (XA_STRING == text.encoding) {
        *name = strdup((char *) text.value);
        ...
        if (Success == XmbTextPropertyToTextList(disp, &text, &list, &nlist) && list) {
            if (0 < nlist && *list) {
...
void subSharedPropertyClass(Display *disp, Window win, char **inst, char **klass) {
    int size = 0;
    char **klasses = NULL;
    ...
    if (inst) {
        *inst = strdup(0 < size ? klasses[0] : "subtle");
    }
    if (klass) {
        *klass = strdup(1 < size ? klasses[1] : "subtle");
    }
...
void subSharedPropertyGeometry(Display *disp, Window win, XRectangle *geometry) {
    Window root = None;
    int x = 0, y = 0;
    ...
    XGetGeometry(disp, win, &root, &x, &y, &width, &height, &bw, &depth);
...
void subSharedPropertyDelete(Display *disp, Window win, Atom prop) {
    assert(win);
...
void subSharedDrawString(Display *disp, GC gc, SubFont *f, Window win, int x, int y, long fg,
                         long bg, const char *text, int len)
{
    XGCValues gvals;
...
void subSharedDrawIcon(Display *disp, GC gc, Window win, int x, int y, int width, int height,
                       long fg, long bg, Pixmap pixmap, int bitmap)
{
    XGCValues gvals;
    ...
    if (bitmap) {
        ...
        XCopyArea(disp, pixmap, win, gc, 0, 0, width, height, x, y);
    }
...
SubFont *subSharedFontNew(Display *disp, const char *name) {
    SubFont *f = NULL;
...
#endif /* HAVE_X11_XFT_XFT_H */
    {
        ...
        if ((xfs = XCreateFontSet(disp, name, &missing, &n, &def))) {
            ...
            /* Font metrics */
            f->height = xfonts[0]->max_bounds.ascent + xfonts[0]->max_bounds.descent + 2;
            f->y = (f->height - 2 + xfonts[0]->max_bounds.ascent) / 2;

            if (missing)
                XFreeStringList(missing); ///< Ignore this
        }
    }
...
void subSharedFontKill(Display *disp, SubFont *f) {
    assert(f);
...
unsigned long subSharedParseColor(Display *disp, char *name) {
    XColor xcolor = {0}; ///< Default color

    assert(name);

    /* Parse and store color */
    if (!XParseColor(disp, DefaultColormap(disp, DefaultScreen(disp)), name, &xcolor)) {
        fprintf(stderr, "<CRITICAL> Failed loading color `%s'\n", name);
    } else if (!XAllocColor(disp, DefaultColormap(disp, DefaultScreen(disp)), &xcolor))
...
KeySym subSharedParseKey(Display *disp, const char *key, unsigned int *code, unsigned int *state,
                         int *mouse)
{
    KeySym sym = NoSymbol;
...
int subSharedStringWidth(Display *disp, SubFont *f, const char *text, int len, int *left,
                         int *right, int center)
{
    ...
    if (text && 0 < len) {
...
#endif /* HAVE_X11_XFT_XFT_H */
        {
            ...
            XmbTextExtents(f->xfs, text, len, &overall_ink, &overall_logical);
...
int subSharedMessage(Display *disp, Window win, char *type, SubMessageData data, int format,
                     int xsync)
{
    int status = 0;
    XEvent ev;
