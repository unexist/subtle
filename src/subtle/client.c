...
/* ClientMask {{{ */
static void ClientMask(XRectangle *geom) {
    XDrawRectangle(subtle->dpy, ROOT, subtle->gcs.invert, geom->x - 1, geom->y - 1, geom->width + 1,
                   geom->height + 1);
} /* }}} */

/* ClientGravity {{{ */
int ClientGravity(void) {
    int grav = 0;
    SubClient *c = NULL;
...
/* ClientBounds {{{ */
static void ClientBounds(SubClient *c, XRectangle *bounds, XRectangle *geom, int adjustx,
                         int adjusty)
{
    DEAD(c);
    assert(c && geom);
        ...
        || c->flags & (SUB_CLIENT_MODE_FLOAT | SUB_CLIENT_MODE_RESIZE)))
    {
        ...
        if (geom->width < c->minw) {
            geom->width = c->minw;
        }
        if (geom->width > maxw) {
            geom->width = maxw;
        }
        if (geom->height < c->minh) {
            geom->height = c->minh;
        }
        if (geom->height > maxh) {
            geom->height = maxh;
...
/* ClientSnap {{{ */
static void ClientSnap(SubClient *c, SubScreen *s, XRectangle *geom) {
    DEAD(c);
    assert(c && s && geom);
    ...
    } else if (abs((s->geom.x + s->geom.width) - (geom->x + geom->width + BORDER(c))) <=
             subtle->snap) {
        geom->x = s->geom.x + s->geom.width - geom->width - BORDER(c);
    }

    /* Snap to screen border when is in snap margin - Y axis */
    if (abs(s->geom.y - geom->y) <= subtle->snap) {
        geom->y = s->geom.y + BORDER(c);
    } else if (abs((s->geom.y + s->geom.height) - (geom->y + geom->height + BORDER(c))) <=
             subtle->snap) {
        geom->y = s->geom.y + s->geom.height - geom->height - BORDER(c);
    }
} /* }}} */

/* ClientResize {{{ */
static void ClientResize(SubClient *c, XRectangle *bounds) {
    assert(c);
    ...
    XMoveResizeWindow(subtle->dpy, c->win, c->geom.x, c->geom.y, c->geom.width, c->geom.height);
} /* }}} */

/* ClientTile {{{ */
static void ClientTile(int gravity, int screen) {
    ...
    for (i = 0; i < subtle->clients->ndata; i++) {
        ...
        if (c->gravityid == gravity && c->screenid == screen && subtle->visible_tags & c->tags
...
/* ClientZaphod {{{ */
static void ClientZaphod(SubClient *c, XRectangle *bounds) {
    ...
    for (i = 0; i < subtle->screens->ndata; i++) {
        ...
        if (s2->flags & flags) {
            ...
            flags &= ~(s2->flags & (SUB_SCREEN_PANEL1 | SUB_SCREEN_PANEL2));
        }
    }
} /* }}} */

/* ClientCompare {{{ */
static int ClientCompare(const void *a, const void *b) {
    int ret = 0, dirret = 0;
    ...
    if (SUB_CLIENT_RESTACK_DOWN == c1->dir) {
        dirret = -1;
    } else if (SUB_CLIENT_RESTACK_UP == c1->dir) {
        dirret = 1;
    } else if (SUB_CLIENT_RESTACK_DOWN == c2->dir) {
        dirret = 1;
    } else if (SUB_CLIENT_RESTACK_UP == c2->dir) {
        dirret = -1;
    }
    ...
    if (c1->flags & SUB_CLIENT_TYPE_DESKTOP) {
        if (c2->flags & SUB_CLIENT_TYPE_DESKTOP) {
            ret = dirret;
        } else ret = 0;
    } else if (c1->flags & SUB_CLIENT_MODE_FULL) {
        if (c2->flags & SUB_CLIENT_MODE_FULL) {
            ret = dirret;
        } else ret = 1;
        ...
        if (c2->flags & SUB_CLIENT_TYPE_DESKTOP) {
            ret = 1;
        } else if (c2->flags & (SUB_CLIENT_MODE_FLOAT | SUB_CLIENT_MODE_FULL)) {
            ret = -1;
        } else ret = dirret;
    }
...
SubClient *subClientNew(Window win) {
    ...
    if (True == attrs.override_redirect) {
        return NULL;
    }
    ...
    XSetWindowBorderWidth(subtle->dpy, c->win, subtle->styles.clients.border.top);
    ...
    subEwmhSetCardinals(c->win, SUB_EWMH_SUBTLE_CLIENT_GRAVITY, (long *) &subtle->gravity, 1);
    subEwmhSetCardinals(c->win, SUB_EWMH_SUBTLE_CLIENT_SCREEN, (long *) &c->screenid, 1);
...
void subClientConfigure(SubClient *c) {
    XConfigureEvent ev;
...
void subClientDimension(int id) {
    int i, j;
...
void subClientFocus(SubClient *c, int warp) {
    SubScreen *s = NULL;
    SubView *v = NULL;
    ...
    /* EWMH: Active window */
    subEwmhSetWindows(ROOT, SUB_EWMH_NET_ACTIVE_WINDOW, subtle->windows.focus, HISTORYSIZE);

    /* EWMH: Current desktop */
    if ((s = SCREEN(subArrayGet(subtle->screens, c->screenid)))) {
        subEwmhSetCardinals(ROOT, SUB_EWMH_NET_CURRENT_DESKTOP, (long *) &s->viewid, 1);
    }
    ...
    /* Hook: Focus */
    subHookCall((SUB_HOOK_TYPE_CLIENT | SUB_HOOK_ACTION_FOCUS), (void *) c);

    /* Warp pointer */
    if (warp && !(subtle->flags & SUB_SUBTLE_SKIP_WARP)) {
        subClientWarp(c);
    }
...
SubClient *subClientNext(int screenid, int jump) {
    int i;
    SubClient *c = NULL;
    ...
    if (1 < subtle->screens->ndata && jump) {
        for (i = subtle->clients->ndata - 1; 0 <= i; i--) {
            ...
            /* Check visibility on current screen */
            if (ALIVE(c) && VISIBLE(c) && c->win != subtle->windows.focus[0]) {
                return c;
            }
        }
    }
...
void subClientWarp(SubClient *c) {
    DEAD(c);
    assert(c);

    /* Move pointer to window center */
    XWarpPointer(subtle->dpy, None, ROOT, 0, 0, 0, 0, c->geom.x + c->geom.width / 2,
...
void subClientDrag(SubClient *c, int mode, int direction) {
    XEvent ev;
    ...
    /* Grab pointer server */
    XGrabPointer(subtle->dpy, c->win, True, GRABMASK, GrabModeAsync, GrabModeAsync, None, cursor,
                 CurrentTime);
        ...
        default: /* {{{ */
            ...
            while (loop) {
                ...
                switch (ev.type) {
                    ...
                    case FocusIn:
                    case FocusOut:
                        break; ///< Ignore focus changes
                    case MotionNotify: /* {{{ */
                        if (mode & (SUB_DRAG_MOVE | SUB_DRAG_RESIZE)) {
                            /* Check values */
                            if (!XYINRECT(ev.xmotion.x_root - dx, ev.xmotion.y_root - dy, s->geom)) {
                                continue;
                            }
                            ...
                            switch (mode) {
                                ...
                                case SUB_DRAG_RESIZE: /* {{{ */
                                    ...
                                    ClientBounds(c, &(s->geom), &geom, (edge & EDGE_LEFT),
                                                 (edge & EDGE_TOP));
                                    break; /* }}} */
                            }
...
void subClientTag(SubClient *c, int tag, int *flags) {
    SubTag *t = NULL;
...
void subClientRetag(SubClient *c, int *flags) {
    int i;
    ...
    if (!(c->flags & SUB_CLIENT_MODE_STICK) && !(*flags & SUB_CLIENT_MODE_STICK)) {
        ...
        if (0 == visible) {
            subClientTag(c, 0, flags); ///< Set default tag
        }
    }
...
void subClientResize(SubClient *c, XRectangle *bounds, int size_hints) {
    DEAD(c);
    assert(c);

    /* Honor size hints */
    if (size_hints)
        ClientBounds(c, bounds, &c->geom, False, False);
    ...
    if (!(c->flags & (SUB_CLIENT_MODE_FULL | SUB_CLIENT_TYPE_DOCK))) {
        ...
        if (!(c->flags & SUB_CLIENT_MODE_FIXED)) {
            if (c->geom.width > bounds->width) {
                c->geom.width = bounds->width;
            }
            if (c->geom.height > bounds->height) {
                c->geom.height = bounds->height;
            }
        }
        ...
        /* Check x and center */
        if (c->geom.x < bounds->x || c->geom.x > maxx || c->geom.x + c->geom.width > maxx) {
            if (c->flags & SUB_CLIENT_MODE_FLOAT) {
        ...
        /* Check y and center */
        if (c->geom.y < bounds->y || c->geom.y > maxy || c->geom.y + c->geom.height > maxy) {
            if (c->flags & SUB_CLIENT_MODE_FLOAT) {
...
void subClientRestack(SubClient *c, int dir) {
    c->dir = dir;
    ...
    subSubtleLogDebugSubtle("Restack: instance=%s, win=%#lx, dir=%s\n", c->instance, c->win,
                            SUB_CLIENT_RESTACK_DOWN == dir ? "down" : "up");
} /* }}} */
...
void subClientArrange(SubClient *c, int gravityid, int screenid) {
    ...
    if (c->flags & SUB_CLIENT_MODE_FULL) {
        ...
        if (c->flags & SUB_CLIENT_MODE_ZAPHOD) {
            XMoveResizeWindow(subtle->dpy, c->win, 0, 0, subtle->width, subtle->height);
        } else {
            XMoveResizeWindow(subtle->dpy, c->win, s->base.x, s->base.y, s->base.width,
                              s->base.height);
        }
        ...
        if (c->flags & SUB_CLIENT_ARRANGE || (-1 != screenid && c->screenid != screenid)) {
            SubScreen *old_s =
                    SCREEN(subArrayGet(subtle->screens, -1 != c->screenid ? c->screenid : 0));
            ...
            XMoveResizeWindow(subtle->dpy, c->win, c->geom.x, c->geom.y, c->geom.width,
                              c->geom.height);
        }
        ...
        if (c->flags & SUB_CLIENT_ARRANGE || c->gravityid != gravityid || c->screenid != screenid) {
                ...
                || (g && g->flags & (SUB_GRAVITY_HORZ | SUB_GRAVITY_VERT)))
            {
                ...
                /* Set size for bounds*/
                if (c->flags & SUB_CLIENT_MODE_ZAPHOD)
                    ClientZaphod(c, &bounds);
                subGravityGeometry(g, &bounds, &c->geom);
                ClientResize(c, &bounds);
            }

            /* EWMH: Gravity */
            subEwmhSetCardinals(c->win, SUB_EWMH_SUBTLE_CLIENT_GRAVITY, (long *) &c->gravityid, 1);

            XSync(subtle->dpy, False); ///< Sync before going on

            /* Hook: Gravity */
            subHookCall((SUB_HOOK_TYPE_CLIENT | SUB_HOOK_ACTION_GRAVITY), (void *) c);
        }
    }
...
void subClientToggle(SubClient *c, int flags, int set_gravity) {
    int nstates = 0;
    Atom states[3] = {None};
        ...
        if (c->flags & SUB_CLIENT_MODE_STICK) {
            ...
            if (!(c->flags & SUB_CLIENT_MODE_STICK_SCREEN)) {
                ...
                if (subtle->flags & SUB_SUBTLE_SKIP_WARP
                    && (focus = CLIENT(subSubtleFind(subtle->windows.focus[0], CLIENTID))) &&
                    VISIBLE(focus))
                {
    ...
    if (flags & SUB_CLIENT_MODE_CENTER) {
        if (c->flags & SUB_CLIENT_MODE_CENTER) {
            ...
            c->geom.x = s->geom.x + (s->geom.width - c->geom.width - 2 * BORDER(c)) / 2;
            c->geom.y = s->geom.y + (s->geom.height - c->geom.height - 2 * BORDER(c)) / 2;
    ...
    c->flags = ((c->flags & ~MODES_ALL) | ((c->flags & MODES_ALL) ^ (flags & MODES_ALL)));

    /* Sort for keeping stacking order */
    if (c->flags & (SUB_CLIENT_MODE_FLOAT | SUB_CLIENT_MODE_FULL | SUB_CLIENT_TYPE_DESKTOP |
                    SUB_CLIENT_TYPE_DOCK))
    ...
    XChangeProperty(subtle->dpy, c->win, subEwmhGet(SUB_EWMH_NET_WM_STATE), XA_ATOM, 32,
...
void subClientSetStrut(SubClient *c) {
                                               ...
                                               subEwmhGet(SUB_EWMH_NET_WM_STRUT), &size)))
    {
        if (4 == size) { ///< Only complete struts
            ...
            subtle->styles.clients.padding.top = MAX(subtle->styles.clients.padding.top, strut[2]);
            subtle->styles.clients.padding.bottom =
...
void subClientSetProtocols(SubClient *c) {
    int i, n = 0;
    Atom *protos = NULL;
    ...
    if (XGetWMProtocols(subtle->dpy, c->win, &protos, &n)) {
        for (i = 0; i < n; i++) {
            switch (subEwmhFind(protos[i])) {
                case SUB_EWMH_WM_TAKE_FOCUS:
                    c->flags |= SUB_CLIENT_FOCUS;
                    break;
                case SUB_EWMH_WM_DELETE_WINDOW:
                    c->flags |= SUB_CLIENT_CLOSE;
                    break;
                default:
                    break;
            }
...
void subClientSetSizeHints(SubClient *c, int *flags) {
    ...
    if (XGetWMNormalHints(subtle->dpy, c->win, hints, &supplied)) {
        ...
        if (hints->flags & PMinSize) {
            ...
            if (hints->min_width) {
                c->minw = c->minw > s->geom.width ? s->geom.width : MAX(MINW, hints->min_width);
            }
            if (hints->min_height) {
                c->minh = c->minh > s->geom.height ? s->geom.height : MAX(MINH, hints->min_height);
            }
        }
        ...
        if (hints->flags & PMaxSize) {
            ...
            if (hints->max_width) {
                c->maxw = hints->max_width > s->geom.width ? s->geom.width : hints->max_width;
            }
        ...
        if ((subtle->flags & SUB_SUBTLE_RESIZE ||
             c->flags & (SUB_CLIENT_MODE_FLOAT | SUB_CLIENT_MODE_RESIZE | SUB_CLIENT_TYPE_DOCK))) {
            /* User/program size */
...
void subClientSetWMHints(SubClient *c, int *flags) {
    XWMHints *hints = NULL;
...
void subClientSetMWMHints(SubClient *c) {
    unsigned long size = 0;
    ...
    if ((hints = (ClientMWMHints *) subSharedPropertyGet(
                 subtle->dpy, c->win, subEwmhGet(SUB_EWMH_MOTIF_WM_HINTS),
...
void subClientSetState(SubClient *c, int *flags) {
    int i;
...
void subClientSetTransient(SubClient *c, int *flags) {
    Window trans = None;
    ...
    if (XGetTransientForHint(subtle->dpy, c->win, &trans)) {
        ...
        *flags |= subtle->flags & SUB_SUBTLE_URGENT ? SUB_CLIENT_MODE_FLOAT | SUB_CLIENT_MODE_URGENT
                                                    : SUB_CLIENT_MODE_FLOAT;
...
void subClientSetType(SubClient *c, int *flags) {
    int i;
    unsigned long size = 0;
    Atom *types = NULL;
...
void subClientClose(SubClient *c) {
    assert(c);
...
void subClientKill(SubClient *c) {
    assert(c);

    /* Hook: Kill */
    subHookCall((SUB_HOOK_TYPE_CLIENT | SUB_HOOK_ACTION_KILL), (void *) c);

    /* Remove _NET_WM_STATE (see EWMH 1.3) */
    subSharedPropertyDelete(subtle->dpy, c->win, subEwmhGet(SUB_EWMH_NET_WM_STATE));
...
void subClientPublish(int restack) {
    int i;
    ...
    subEwmhSetWindows(ROOT, SUB_EWMH_NET_CLIENT_LIST_STACKING, wins, subtle->clients->ndata);
