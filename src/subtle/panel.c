...
#define STYLE_FONT_Y(S, F)                                                                         \
    ((subtle->ph - STYLE_HEIGHT((S))) - F->height) / 2 + F->y + STYLE_TOP((S));

/* PanelDrawRect {{{ */
static void PanelDrawRect(Drawable drawable, int x, int width, SubStyle *s) {
    if (0 >= width) {
        return;
    }
    ...
    XFillRectangle(subtle->dpy, drawable, subtle->gcs.draw, x + s->margin.left, s->margin.top,
                   width - mw, subtle->ph - mh);

    /* Borders: Top */
    XSetForeground(subtle->dpy, subtle->gcs.draw, s->top);
    XFillRectangle(subtle->dpy, drawable, subtle->gcs.draw, x + s->margin.left, s->margin.top,
                   width - mw, s->border.top);

    /* Borders: Right */
    XSetForeground(subtle->dpy, subtle->gcs.draw, s->right);
    XFillRectangle(subtle->dpy, drawable, subtle->gcs.draw,
                   x + width - s->border.right - s->margin.right, s->margin.top, s->border.right,
                   subtle->ph - mh);

    /* Borders: Bottom */
    XSetForeground(subtle->dpy, subtle->gcs.draw, s->bottom);
    XFillRectangle(subtle->dpy, drawable, subtle->gcs.draw, x + s->margin.left,
                   subtle->ph - s->border.bottom - s->margin.bottom, width - mw, s->border.bottom);

    /* Borders: Left */
    XSetForeground(subtle->dpy, subtle->gcs.draw, s->left);
    XFillRectangle(subtle->dpy, drawable, subtle->gcs.draw, x + s->margin.left, s->margin.top,
                   s->border.left, subtle->ph - mh);
} /* }}} */

/* PanelDrawSeparator {{{ */
static void PanelDrawSeparator(int x, SubStyle *s, Drawable drawable) {
    /* Set window background and border*/
    PanelDrawRect(drawable, x, s->separator->width, s);

    subSharedDrawString(subtle->dpy, subtle->gcs.draw, s->font, drawable, x + STYLE_LEFT((*s)),
                        s->font->y + STYLE_TOP((*s)), s->fg, s->bg, s->separator->string,
                        strlen(s->separator->string));
} /* }}} */

/* PanelClientModes {{{ */
void PanelClientModes(SubClient *c, char *buf, int *width) {
    int x = 0;
    ...
    *width = subSharedStringWidth(subtle->dpy, subtle->styles.title.font, buf, strlen(buf), NULL,
                                  NULL, True);
} /* }}} */

/* PanelViewStyle {{{ */
static void PanelViewStyle(SubView *v, int idx, int focus, SubStyle *s) {
    /* Select style */
    if (subtle->styles.views.styles) {
        ...
        subStyleMerge(s, &subtle->styles.views);
    }
} /* }}} */

/* PanelSubletStyle {{{ */
static SubStyle *PanelSubletStyle(SubPanel *p) {
    SubStyle *s = NULL;
...
void subPanelUpdate(SubPanel *p) {
    assert(p);

    /* Handle panel item type */
    switch (p->flags & (SUB_PANEL_TRAY | SUB_PANEL_ICON | SUB_PANEL_KEYCHAIN | SUB_PANEL_SUBLET |
                        SUB_PANEL_TITLE | SUB_PANEL_VIEWS))
    {
        case SUB_PANEL_TRAY: /* {{{ */
            ...
            if (0 < subtle->trays->ndata) {
                ...
                for (i = 0; i < subtle->trays->ndata; i++) {
                    ...
                    if (t->flags & SUB_TRAY_DEAD) {
                        continue;
                    }

                    XMapWindow(subtle->dpy, t->win);
                    XMoveResizeWindow(subtle->dpy, t->win, p->width, 0, t->width,
                ...
                XUnmapWindow(subtle->dpy, subtle->windows.tray);
            }
            break; /* }}} */
        case SUB_PANEL_ICON: /* {{{ */
        ...
        case SUB_PANEL_KEYCHAIN: /* {{{ */
            ...
            if (p->keychain && p->keychain->keys) {
                ...
                p->width = subSharedStringWidth(subtle->dpy, subtle->styles.separator.font,
                                                p->keychain->keys, p->keychain->len, NULL, NULL,
                                                True) +
            ...
            if (0 < subtle->clients->ndata) {
                ...
                if ((c = CLIENT(subSubtleFind(subtle->windows.focus[0], CLIENTID)))) {
                    ...
                    if (!(c->flags & SUB_CLIENT_TYPE_DESKTOP)) {
                        ...
                        /* Font offset, panel border and padding */
                        p->width = subSharedStringWidth(subtle->dpy, subtle->styles.title.font,
                                                        c->name,
                                                        /* Limit string length */
                                                        len > subtle->styles.clients.right
                                                                ? subtle->styles.clients.right
                                                                : len,
                                                        NULL, NULL, True) +
        ...
        case SUB_PANEL_VIEWS: /* {{{ */
            ...
            if (0 < subtle->views->ndata) {
                ...
                for (i = 0; i < subtle->views->ndata; i++) {
                    ...
                    /* Skip dynamic views */
                    if (v->flags & SUB_VIEW_DYNAMIC && !(subtle->client_tags & v->tags))
                        continue;
                    ...
                    if (v->flags & SUB_VIEW_ICON_ONLY) {
                        ...
                        v->width = subSharedStringWidth(subtle->dpy, s.font, v->name,
                                                        strlen(v->name), NULL, NULL, True) +
                ...
                if (subtle->styles.view_sep) {
                    p->width +=
                            (subtle->views->ndata - 1) * subtle->styles.view_sep->separator->width;
                }
            }
            break; /* }}} */
    }
...
void subPanelRender(SubPanel *p, Drawable drawable) {
    assert(p);
    ...
    switch (p->flags & (SUB_PANEL_TRAY | SUB_PANEL_ICON | SUB_PANEL_KEYCHAIN | SUB_PANEL_SUBLET |
        ...
        case SUB_PANEL_ICON: /* {{{ */
            {
                ...
                y = subtle->styles.separator.font->y + STYLE_TOP(subtle->styles.separator);
                icony = p->icon->height > y ? subtle->styles.separator.margin.top : y - p->icon->height;

                subSharedDrawIcon(subtle->dpy, subtle->gcs.draw, drawable,
                                  p->x + 2 + subtle->styles.separator.padding.left, icony,
        ...
        case SUB_PANEL_KEYCHAIN: /* {{{ */
            if (p->keychain && p->keychain->keys) {
                subSharedDrawString(subtle->dpy, subtle->gcs.draw, subtle->styles.separator.font,
        ...
        case SUB_PANEL_SUBLET: /* {{{ */
            {
                ...
                /* Render text parts */
                subTextRender(p->sublet->text, s->font, subtle->gcs.draw, drawable,
                              p->x + STYLE_LEFT((*s)), y, s->fg, s->icon, s->bg);
            }
            break; /* }}} */
        case SUB_PANEL_TITLE: /* {{{ */
            if (0 < subtle->clients->ndata) {
                ...
                if ((c = CLIENT(subSubtleFind(subtle->windows.focus[0], CLIENTID))) &&
                    ...
                    subSharedDrawString(subtle->dpy, subtle->gcs.draw, subtle->styles.title.font,
                                        drawable, x, y, subtle->styles.title.fg,
                                        subtle->styles.title.bg, buf, strlen(buf));

                    subSharedDrawString(
                            subtle->dpy, subtle->gcs.draw, subtle->styles.title.font, drawable,
                            x + width, y, subtle->styles.title.fg, subtle->styles.title.bg, c->name,
                            /* Limit string length */
                            len > subtle->styles.clients.right ? subtle->styles.clients.right
                                                               : len);
                }
            }
            break; /* }}} */
            ...
            if (0 < subtle->views->ndata) {
                ...
                for (i = 0; i < subtle->views->ndata; i++) {
                    ...
                    if (v->flags & SUB_VIEW_ICON) {
                        ...
                        subSharedDrawIcon(subtle->dpy, subtle->gcs.draw, drawable, vx + x, icony,
                                          v->icon->width, v->icon->height, s.icon, s.bg,
                                          v->icon->pixmap, v->icon->bitmap);
                    }
                    ...
                    if (!(v->flags & SUB_VIEW_ICON_ONLY)) {
                        ...
                        subSharedDrawString(subtle->dpy, subtle->gcs.draw, s.font, drawable, vx + x,
                                            y, s.fg, s.bg, v->name, strlen(v->name));
                    }
    ...
    if (p->flags & SUB_PANEL_SEPARATOR2 && subtle->styles.separator.separator) {
        SubStyle *s = p->flags & SUB_PANEL_SUBLET && subtle->styles.sublet_sep
                              ? subtle->styles.sublet_sep
                              : &subtle->styles.separator;
...
int subPanelCompare(const void *a, const void *b) {
    SubPanel *p1 = *(SubPanel **) a, *p2 = *(SubPanel **) b;

    assert(a && b);

    /* Include only interval sublets */
    if (!(p1->sublet->flags & (SUB_SUBLET_INTERVAL))) {
        return 1;
    }
    if (!(p2->sublet->flags & (SUB_SUBLET_INTERVAL))) {
        return -1;
    }
...
void subPanelAction(SubArray *panels, int type, int x, int y, int button, int bottom) {
    int i;
    ...
    for (i = 0; i < panels->ndata; i++) {
        ...
        if (p->flags & type && x >= p->x && x <= p->x + p->width) {
            ...
            if ((bottom && !(p->flags & SUB_PANEL_BOTTOM))
                || (!bottom && p->flags & SUB_PANEL_BOTTOM)) {
                continue;
            }
                ...
                case SUB_PANEL_SUBLET: /* {{{ */
                    ...
                    switch (type) {
                        ...
                        case SUB_PANEL_DOWN: {
                            ...
                            subRubyCall(SUB_CALL_DOWN, p->sublet->instance, (void *) &args);
                        } break;
                    }
                ...
                case SUB_PANEL_VIEWS: /* {{{ */
                    {
                        ...
                        for (j = 0; j < subtle->views->ndata; j++) {
                            ...
                            if (x >= vx && x <= vx + v->width) {
                                /* FIXME */
                                int sid = subArrayIndex(subtle->screens, (void *) p->screen);
...
void subPanelGeometry(SubPanel *p, SubStyle *s, XRectangle *geom) {
    assert(p && s && geom);

    /* Calculate panel geometry without style values */
    geom->x = p->x + STYLE_LEFT((*s));
    geom->y = p->flags & SUB_PANEL_BOTTOM ? p->screen->geom.y + p->screen->geom.height - subtle->ph
                                          : STYLE_TOP((*s));
...
void subPanelPublish(void) {
    int i = 0, j = 0, idx = 0;
    char **sublets = NULL, buf[30] = {0};
    XRectangle geom = {0};

    /* Alloc space */
    sublets = (char **) subSharedMemoryAlloc(subtle->sublets->ndata, sizeof(char *));
        ...
        if (s->panels) {
            for (j = 0; j < s->panels->ndata; j++) {
                ...
                if (p->flags & SUB_PANEL_SUBLET && !(p->flags & SUB_PANEL_COPY)) {
                    ...
                    /* Add gravity to list */
                    snprintf(buf, sizeof(buf), "%dx%d+%d+%d#%s", geom.x, geom.y, geom.width,
                             geom.height, p->sublet->name);

                    sublets[idx] = (char *) subSharedMemoryAlloc(strlen(buf) + 1, sizeof(char));
    ...
    subSharedPropertySetStrings(subtle->dpy, ROOT, subEwmhGet(SUB_EWMH_SUBTLE_SUBLET_LIST), sublets,
...
void subPanelKill(SubPanel *p) {
    assert(p);

    /* Handle panel item type */
    switch (p->flags & (SUB_PANEL_COPY | SUB_PANEL_ICON | SUB_PANEL_KEYCHAIN | SUB_PANEL_SUBLET |
                        SUB_PANEL_TRAY))
    {
        case SUB_PANEL_COPY:
            break;
