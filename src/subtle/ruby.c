...
/* RubyBacktrace {{{ */
static void RubyBacktrace(void) {
    VALUE lasterr = Qnil;
...
/* RubyReceiver {{{ */
static int RubyReceiver(unsigned long instance, unsigned long meth) {
    VALUE receiver = Qnil;
...
/* RubyFont {{{ */
static SubFont *RubyFont(const char *fontname) {
    SubFont *f = NULL;
...
static VALUE RubySubtleToSubtlext(void *data) {
    SubClient *c = NULL;
    VALUE object = Qnil;
    ...
    if ((c = CLIENT(data))) {
        ...
        if (c->flags & SUB_TYPE_CLIENT) { /* {{{ */
            ...
            value = rb_funcall(klass, rb_intern("new"), 4, INT2FIX(c->geom.x), INT2FIX(c->geom.y),
                               INT2FIX(c->geom.width), INT2FIX(c->geom.height));
            ...
            if (-1 != c->gravityid) {
                ...
                rb_funcall(value, rb_intern("geometry="), 4, INT2FIX(g->geom.x), INT2FIX(g->geom.y),
                           INT2FIX(g->geom.width), INT2FIX(g->geom.height));
            } else {
                value = Qnil;
            }
            ...
            object = rb_funcall(klass, rb_intern("new"), 1, INT2FIX(id));
            geom = rb_funcall(klass_geom, rb_intern("new"), 4, INT2FIX(s->geom.x),
                              INT2FIX(s->geom.y), INT2FIX(s->geom.width), INT2FIX(s->geom.height));
...
/* RubyIconToIcon {{{ */
static void RubyIconToIcon(VALUE icon, SubIcon *i) {
...
/* RubySymbolToFlag {{{ */
static void RubySymbolToFlag(VALUE sym, int *flags) {
    /* Translate symbols to flags */
    if (CHAR2SYM("name") == sym) {
        (*flags) |= SUB_TAG_MATCH_NAME;
    } else if (CHAR2SYM("instance") == sym) {
        (*flags) |= SUB_TAG_MATCH_INSTANCE;
    } else if (CHAR2SYM("class") == sym) {
        (*flags) |= SUB_TAG_MATCH_CLASS;
    } else if (CHAR2SYM("role") == sym) {
        (*flags) |= SUB_TAG_MATCH_ROLE;
    } else if (CHAR2SYM("type") == sym) {
        (*flags) |= SUB_TAG_MATCH_TYPE;
    }

    /* Window types */
    else if (CHAR2SYM("normal") == sym) {
        (*flags) |= SUB_CLIENT_TYPE_NORMAL;
    } else if (CHAR2SYM("desktop") == sym) {
        (*flags) |= SUB_CLIENT_TYPE_DESKTOP;
    } else if (CHAR2SYM("dock") == sym) {
        (*flags) |= SUB_CLIENT_TYPE_DOCK;
    } else if (CHAR2SYM("toolbar") == sym) {
        (*flags) |= SUB_CLIENT_TYPE_TOOLBAR;
    } else if (CHAR2SYM("splash") == sym) {
        (*flags) |= SUB_CLIENT_TYPE_SPLASH;
    } else if (CHAR2SYM("dialog") == sym) {
        (*flags) |= SUB_CLIENT_TYPE_DIALOG;
    }

    /* Tag modes */
    else if (CHAR2SYM("borderless") == sym) {
        (*flags) |= SUB_CLIENT_MODE_BORDERLESS;
    } else if (CHAR2SYM("centered") == sym) {
        (*flags) |= SUB_CLIENT_MODE_CENTER;
    } else if (CHAR2SYM("fixed") == sym) {
        (*flags) |= SUB_CLIENT_MODE_FIXED;
    } else if (CHAR2SYM("floating") == sym) {
        (*flags) |= SUB_CLIENT_MODE_FLOAT;
    } else if (CHAR2SYM("fullscreen") == sym) {
        (*flags) |= SUB_CLIENT_MODE_FULL;
    } else if (CHAR2SYM("resize") == sym) {
        (*flags) |= SUB_CLIENT_MODE_RESIZE;
    } else if (CHAR2SYM("sticky") == sym) {
        (*flags) |= SUB_CLIENT_MODE_STICK;
    } else if (CHAR2SYM("urgent") == sym) {
        (*flags) |= SUB_CLIENT_MODE_URGENT;
    } else if (CHAR2SYM("zaphod") == sym) {
        (*flags) |= SUB_CLIENT_MODE_ZAPHOD;
    }

    /* View modes */
    else if (CHAR2SYM("dynamic") == sym) {
        (*flags) |= SUB_VIEW_DYNAMIC;
    } else if (CHAR2SYM("icons_only") == sym) {
        (*flags) |= SUB_VIEW_ICON_ONLY;
    }
} /* }}} */

/* RubyArrayToArray {{{ */
static void RubyArrayToArray(VALUE ary, int *values, int len) {
    /* Check value type */
    if (T_ARRAY == rb_type(ary)) {
        ...
        for (i = 0; i < len; i++) {
            ...
            switch (rb_type(value = rb_ary_entry(ary, i))) {
                case T_FIXNUM:
                    values[i] = FIX2INT(value);
                    break;
                case T_FLOAT:
                    values[i] = FIX2INT(rb_to_int(value));
                    break;
                default:
                    values[i] = 0;
                    break;
...
/* RubyArrayToGeometry {{{ */
static void RubyArrayToGeometry(VALUE ary, XRectangle *geometry) {
    int values[4] = {0};
...
/* RubyArrayToSides {{{ */
static void RubyArrayToSides(VALUE ary, SubSides *sides) {
    int values[4] = {0};
...
/* RubyValueToGravity {{{ */
static int RubyValueToGravity(VALUE value) {
    int gravity = -1;
...
static void RubyValueToGravityString(VALUE value, char **string) {
    /* Check value type */
        ...
        *string = (char *) subSharedMemoryAlloc(size + 1, sizeof(char));
        ...
        for (i = 0, j = 0; Qnil != (entry = rb_ary_entry(value, i)); i++) {
            ...
            if (-1 != (id = RubyValueToGravity(entry))) {
                ...
                subSubtleLogWarn("Cannot find gravity `%s'\n", SYM2CHAR(entry));
            }
        }
    }
} /* }}} */
...
/* RubyValueToIcon {{{ */
static VALUE RubyValueToIcon(VALUE value) {
...
/* RubyHashToColor {{{ */
static void RubyHashToColor(VALUE hash, const char *key, long *col) {
    VALUE value = Qnil;
...
/* RubyHashToInt {{{ */
static void RubyHashToInt(VALUE hash, const char *key, int *val) {
    VALUE value = Qnil;
...
/* RubyHashToBorder {{{ */
static void RubyHashToBorder(VALUE hash, const char *key, long *col, int *bw) {
    VALUE value = Qnil;
...
/* RubyHashToSides {{{ */
static void RubyHashToSides(VALUE params, const char *name, SubSides *s) {
    VALUE value = Qnil;
...
/* RubyEvalHook {{{ */
static void RubyEvalHook(VALUE event, VALUE proc) {
    int i;
    SubHook *h = NULL;
...
/* RubyEvalGrab {{{ */
static void RubyEvalGrab(VALUE keys, VALUE value) {
    int type = -1;
    SubData data = {None};
    ...
    if (T_STRING == rb_type(keys)) {
                ...
                if ((g = subGrabNew(tok, &duplicate))) {
                    ...
                    if (0 < ntok) {
                        ...
                        if (NULL == prev->keys) {
                            ...
                            if (1 == ntok)
                                prev->flags |= SUB_GRAB_CHAIN_START;

                            prev->keys = subArrayNew();
                        }

                        /* Mark grabs w/o action as chain link */
                        if (0 == (g->flags & ~(SUB_TYPE_GRAB | SUB_GRAB_KEY | SUB_GRAB_MOUSE |
                                               SUB_GRAB_CHAIN_LINK))) {
        ...
        rb_raise(rb_eArgError, "Unknown value type for grab");
    }
} /* }}} */

/* RubyEvalPanel {{{ */
void RubyEvalPanel(VALUE ary, int position, SubScreen *s) {
    /* Check value type */
    if (T_ARRAY == rb_type(ary)) {
        int i, j, flags = 0;
        ...
        for (i = 0; Qnil != (entry = rb_ary_entry(ary, i)); ++i) {
            ...
            if (entry == spacer || entry == center || entry == separator) {
                ...
                if (!subtle->panels.tray.screen) {
                    ...
                    XReparentWindow(subtle->dpy, subtle->windows.tray, panel, 0, 0);
                }
            } else if (entry == views) {
                ...
                if (!subtle->panels.keychain.screen) {
                    ...
                    p->keychain = KEYCHAIN(subSharedMemoryAlloc(1, sizeof(SubKeychain)));
                }
                ...
                if (rb_obj_is_instance_of(entry, rb_const_get(subtlext, rb_intern("Icon")))) {
                ...
                for (j = 0; j < subtle->sublets->ndata; j++) {
                    ...
                    if (entry == CHAR2SYM(p2->sublet->name)) {
                        ...
                        if (p2->screen) {
                            ...
                            p->flags |= (p2->flags & (SUB_PANEL_SUBLET | SUB_PANEL_DOWN |
            ...
            if (p) {
                p->flags |= SUB_SCREEN_PANEL2 == position ? (flags | SUB_PANEL_BOTTOM)
                                                          : flags; ///< Mark for bottom panel
                p->screen = s;
...
/* RubyEvalPanelConfig {{{ */
static void RubyEvalPanelConfig(int flag, VALUE config) {
    int i;
...
/* RubyEvalConfig {{{ */
static void RubyEvalConfig(void) {
    int i;
    ...
    for (i = 0; i < subtle->tags->ndata; i++) {
        ...
        if (t->flags & SUB_CLIENT_MODE_STICK) {
            if (-1 != t->screenid && (0 > t->screenid || subtle->screens->ndata <= t->screenid)) {
...
/* RubyEvalStyle {{{ */
static void RubyEvalStyle(VALUE name, SubStyle *style, VALUE params) {
    int bw = -1;
    long border = -1;
    VALUE value = Qnil;
    ...
    if (CHAR2SYM("clients") == name) {
        ...
        if (CHAR2SYM("panel_top") == name || CHAR2SYM("panel_bottom") == name) {
            ...
            if (T_ARRAY == rb_type(value = rb_hash_lookup(params, CHAR2SYM("screen")))) {
            ...
            RubyHashToColor(params, "icon", &style->icon);
        }
        ...
        /* Get borders */
        RubyHashToBorder(params, "border_top", &style->top, &style->border.top);
        RubyHashToBorder(params, "border_right", &style->right, &style->border.right);
        RubyHashToBorder(params, "border_bottom", &style->bottom, &style->border.bottom);
        RubyHashToBorder(params, "border_left", &style->left, &style->border.left);
        ...
        /* Style font */
        if (T_STRING == rb_type(value = rb_hash_lookup(params, CHAR2SYM("font"))) && !style->font) {
        ...
        /* Style separator */
        if (T_STRING == rb_type(value = rb_hash_lookup(params, CHAR2SYM("separator"))) &&
            !style->separator)
        {
            style->flags |= SUB_STYLE_SEPARATOR;

            /* Create new separator */
            style->separator = (SubSeparator *) subSharedMemoryAlloc(1, sizeof(SubSeparator));
...
/* RubyForeachMatcher {{{ */
static int RubyForeachMatcher(VALUE key, VALUE value, VALUE data) {
    int type = 0;
    ...
    if (0 < type) {
        subTagMatcherAdd(TAG(rargs[0]), type, NIL_P(regex) ? NULL : RSTRING_PTR(regex),
                         0 < rargs[1]++);
    }
...
/* RubyForeachStyle {{{ */
static int RubyForeachStyle(VALUE key, VALUE value, VALUE data) {
    SubStyle *s = (SubStyle *) data, *style = NULL;

    if (key == Qundef || NIL_P(value)) {
        return ST_CONTINUE;
    }
    ...
    if (&subtle->styles.views == s) {
        if (CHAR2SYM("urgent") == key) {
            subtle->styles.urgent = style;
        } else if (CHAR2SYM("occupied") == key) {
            subtle->styles.occupied = style;
        } else if (CHAR2SYM("focus") == key) {
            subtle->styles.focus = style;
        } else if (CHAR2SYM("visible") == key) {
            subtle->styles.visible = style;
        } else if (CHAR2SYM("separator") == key) {
            subtle->styles.view_sep = style;
        }
    } else if (&subtle->styles.sublets == s) {
        if (CHAR2SYM("separator") == key) {
            subtle->styles.sublet_sep = style;
        }
    }
...
static VALUE RubyWrapLoadSubtlext(VALUE data) {
    return rb_require("subtle/subtlext");
} /* }}} */

/* RubyWrapLoadPanels {{{ */
static VALUE RubyWrapLoadPanels(VALUE data) {
    int i;
    SubScreen *s = NULL;
            ...
            for (j = 0; j < s->panels->ndata; j++) {
                ...
                if (p->flags & SUB_PANEL_SUBLETS) {
                    ...
                    if ((sublet = PANEL(subArrayGet(s->panels, j)))) {
                        sublet->flags |= (p->flags & (SUB_PANEL_BOTTOM | SUB_PANEL_SPACER1 |
                                                      SUB_PANEL_SEPARATOR1 | SUB_PANEL_CENTER));
                    }
...
/* RubyWrapCall {{{ */
static VALUE RubyWrapCall(VALUE data) {
    ...
    switch ((int) rargs[0]) {
        ...
        case SUB_CALL_DATA: /* {{{ */
            {
                ...
                if ((list = subSharedPropertyGetStrings(subtle->dpy, ROOT, prop, &nlist))) {
                    if (list && 0 < nlist) {
                ...
                /* Finally call method */
                rb_funcall(rargs[1], meth, MINMAX(rb_obj_method_arity(rargs[1], meth), 1, 2), rargs[1],
                           str);
            }
            break; /* }}} */
        case SUB_CALL_WATCH: /* {{{ */
        ...
        case SUB_CALL_DOWN: /* {{{ */
            {
                ...
                rb_funcall(rargs[1], meth, MINMAX(rb_obj_method_arity(rargs[1], meth), 1, 4), rargs[1],
                           INT2FIX(args[0]), INT2FIX(args[1]), INT2FIX(args[2]));
            }
        break; /* }}} */
        case SUB_CALL_OVER: /* {{{ */
        ...
        default: /* {{{ */
            ...
            if (rb_obj_is_instance_of(rargs[1], rb_cMethod)) {
                ...
                arity = FIX2INT(rb_funcall(rargs[1], rb_intern("arity"), 0, NULL));
                ...
                rb_funcall(rargs[1], rb_intern("call"), MINMAX(rb_proc_arity(rargs[1]), 0, 1),
                           RubySubtleToSubtlext((VALUE *) rargs[2]));
            }
            break; /* }}} */
    }
...
/* RubyWrapRelease {{{ */
static VALUE RubyWrapRelease(VALUE value) {
...
static VALUE RubyWrapSubletConfig(VALUE data) {
    ...
    if (T_HASH == rb_type(hash = rb_hash_lookup(config_sublets, rargs[0]))) {
        ...
        /* Set sublet style */
        if (T_SYMBOL == rb_type(value = rb_hash_lookup(hash, CHAR2SYM("style")))) {
            value = rb_sym_to_s(value);

            subStyleFind(&subtle->styles.sublets, RSTRING_PTR(value), &s->styleid);
        }
    }
...
static VALUE RubyObjectDispatcher(VALUE self, VALUE missing) {
    VALUE ret = Qnil;
...
static VALUE RubyOptionsDispatcher(int argc, VALUE *argv, VALUE self) {
    VALUE ret = Qnil, missing = Qnil, args[4] = {Qnil};

    rb_scan_args(argc, argv, "14", &missing, &args[0], &args[1], &args[2], &args[3]);
...
static VALUE RubyOptionsGravity(VALUE self, VALUE gravity) {
...
static VALUE RubyOptionsStyle(VALUE self, VALUE name) {
...
static VALUE RubyConfigMissing(int argc, VALUE *argv, VALUE self) {
    char *name = NULL;
...
static VALUE RubyConfigAdded(VALUE self, VALUE name) {
...
static VALUE RubyConfigLoadConfig(VALUE self, VALUE file) {
    ...
    if (-1 == access(RSTRING_PTR(file), R_OK)) {
        int len = 0;
        char *home = NULL, *dirs = NULL, *tok = NULL, tokens[200] = {0}, *tokensp = tokens;

        /* Combine XDG paths */
        if ((home = getenv("XDG_CONFIG_HOME"))) {
            len += snprintf(tokens, sizeof(tokens), "%s", home);
        } else {
            len += snprintf(tokens, sizeof(tokens), "%s/.config", getenv("HOME"));
        }

        if ((dirs = getenv("XDG_CONFIG_DIRS"))) {
            len += snprintf(tokens + len, sizeof(tokens), ":%s", dirs);
        } else {
            len += snprintf(tokens + len, sizeof(tokens), ":%s/%s", "/etc/xdg", PKG_NAME);
        }

        if ((home = getenv("XDG_DATA_HOME"))) {
            snprintf(buf, sizeof(buf), "%s:%s/%s/sublets", tokens, home, PKG_NAME);
        } else {
            snprintf(buf, sizeof(buf), "%s:%s/.local/share/%s/sublets", tokens, getenv("HOME"),
                     PKG_NAME);
        }
        ...
        while ((tok = strsep(&tokensp, ":"))) {
            ...
            if (-1 != access(buf, R_OK)) {
                ...
                snprintf(buf, sizeof(buf), "%s/%s/%s", tok, PKG_NAME, RSTRING_PTR(file));
        ...
        snprintf(buf, sizeof(buf), "%s", RSTRING_PTR(file));
    }
...
static VALUE RubyConfigSet(VALUE self, VALUE option, VALUE value) {
    /* Check value type */
    if (T_SYMBOL == rb_type(option)) {
        switch (rb_type(value)) {
            case T_FIXNUM: /* {{{ */
                if (CHAR2SYM("step") == option || CHAR2SYM("increase_step") == option) {
                    if (!(subtle->flags & SUB_SUBTLE_CHECK)) {
                        subtle->step = FIX2INT(value);
                    }
                } else if (CHAR2SYM("snap") == option || CHAR2SYM("border_snap") == option) {
                    if (!(subtle->flags & SUB_SUBTLE_CHECK)) {
                        subtle->snap = FIX2INT(value);
                    }
                } else if (CHAR2SYM("gravity") == option || CHAR2SYM("default_gravity") == option) {
                    if (!(subtle->flags & SUB_SUBTLE_CHECK)) {
                ...
                break; /* }}} */
            case T_SYMBOL: /* {{{ */
                if (CHAR2SYM("gravity") == option || CHAR2SYM("default_gravity") == option) {
                    if (!(subtle->flags & SUB_SUBTLE_CHECK)) {
                        subtle->gravity = value; ///< Store for later
                    }
                } else {
            ...
            case T_TRUE:
            case T_FALSE: /* {{{ */
                if (CHAR2SYM("urgent") == option || CHAR2SYM("urgent_dialogs") == option) {
                    if (!(subtle->flags & SUB_SUBTLE_CHECK) && Qtrue == value) {
                        subtle->flags |= SUB_SUBTLE_URGENT;
                    }
                } else if (CHAR2SYM("resize") == option || CHAR2SYM("honor_size_hints") == option) {
                    if (!(subtle->flags & SUB_SUBTLE_CHECK) && Qtrue == value) {
                        subtle->flags |= SUB_SUBTLE_RESIZE;
                    }
                } else if (CHAR2SYM("tiling") == option || CHAR2SYM("gravity_tiling") == option) {
                    if (!(subtle->flags & SUB_SUBTLE_CHECK) && Qtrue == value) {
            ...
            case T_STRING: /* {{{ */
                if (CHAR2SYM("wmname") == option) {
                    ...
                    if (!(subtle->flags & SUB_SUBTLE_CHECK)) {
                        ...
                        subEwmhSetWindows(ROOT, SUB_EWMH_NET_SUPPORTING_WM_CHECK, &root, 1);
                        subEwmhSetString(root, SUB_EWMH_NET_WM_NAME, RSTRING_PTR(value));
                    }
                } else {
                    subSubtleLogWarn("Unknown option `:%s'\n", SYM2CHAR(option));
                }
                break; /* }}} */
            default:
                rb_raise(rb_eArgError, "Unexpected value type for option `%s'", SYM2CHAR(option));
        }
    } else {
        rb_raise(rb_eArgError, "Unexpected value type for set `%s'", rb_obj_classname(option));
    }
...
static VALUE RubyConfigStyle(VALUE self, VALUE name) {
    rb_need_block();
    ...
    if (T_SYMBOL == rb_type(name)) {
        ...
        /* Select style struct */
        if (CHAR2SYM("all") == name) {
            style = &subtle->styles.all;
        } else if (CHAR2SYM("views") == name) {
            style = &subtle->styles.views;
        } else if (CHAR2SYM("title") == name) {
            style = &subtle->styles.title;
        } else if (CHAR2SYM("sublets") == name) {
            style = &subtle->styles.sublets;
        } else if (CHAR2SYM("separator") == name) {
            style = &subtle->styles.separator;
        } else if (CHAR2SYM("clients") == name) {
            style = &subtle->styles.clients;
        } else if (CHAR2SYM("panel_top") == name) {
            style = &subtle->styles.panel_top;
        } else if (CHAR2SYM("panel_bottom") == name) {
            style = &subtle->styles.panel_bot;
        } else if (CHAR2SYM("tray") == name) {
            style = &subtle->styles.tray;
        } else {
        ...
        rb_raise(rb_eArgError, "Unexpected value type for style `%s'", rb_obj_classname(name));
    }
...
static VALUE RubyConfigGravity(int argc, VALUE *argv, VALUE self) {
    ...
    if (T_SYMBOL == rb_type(name) && T_ARRAY == rb_type(value)) {
        ...
        if (!(subtle->flags & SUB_SUBTLE_CHECK)) {
            ...
            if ((g = subGravityNew(SYM2CHAR(name), &geometry))) {
                ...
                if (T_SYMBOL == rb_type(tile)) {
                    if (CHAR2SYM("horz") == tile) {
                        g->flags |= SUB_GRAVITY_HORZ;
                    } else if (CHAR2SYM("vert") == tile) {
                        g->flags |= SUB_GRAVITY_VERT;
                    }
                }
        ...
        rb_raise(rb_eArgError, "Unexpected value type for gravity `%s'", rb_obj_classname(value));
    }
...
static VALUE RubyConfigGrab(int argc, VALUE *argv, VALUE self) {
    VALUE chain = Qnil, value = Qnil;

    rb_scan_args(argc, argv, "11", &chain, &value);

    if (rb_block_given_p()) {
        value = rb_block_proc(); ///< Get proc
    }
...
static VALUE RubyConfigTag(int argc, VALUE *argv, VALUE self) {
    ...
    if (rb_block_given_p()) {
        ...
        /* Set gravity */
        if (T_SYMBOL == rb_type(value = rb_hash_lookup(params, CHAR2SYM("gravity"))) ||
            T_FIXNUM == rb_type(value) || T_ARRAY == rb_type(value)) {
            flags |= SUB_TAG_GRAVITY;
            gravityid = value; ///< Lazy eval
        }

        /* Set geometry */
        if (T_ARRAY == rb_type(value = rb_hash_lookup(params, CHAR2SYM("geometry")))) {
            flags |= SUB_TAG_GEOMETRY;
            RubyArrayToGeometry(value, &geom);
        }

        /* Set position */
        if (T_ARRAY == rb_type(value = rb_hash_lookup(params, CHAR2SYM("position")))) {
            flags |= SUB_TAG_POSITION;
            RubyArrayToGeometry(value, &geom);
        }

        /* Set window type */
        if (T_SYMBOL == rb_type(value = rb_hash_lookup(params, CHAR2SYM("type")))) {
            RubySymbolToFlag(value, &flags);
        }

        /* Set modes */
        if (T_SYMBOL == rb_type(value = rb_hash_lookup(params, CHAR2SYM("set")))) {
            RubySymbolToFlag(value, &flags);
    ...
    if (T_STRING == rb_type(name)) {
        ...
        if (!(subtle->flags & SUB_SUBTLE_CHECK)) {
            ...
            if ((t = subTagNew(RSTRING_PTR(name), &duplicate)) && False == duplicate) {
                int i;
                ...
                switch (rb_type(match)) {
                    case T_HASH:
                        ...
                        break;
                    case T_ARRAY:
                        for (i = 0; T_HASH == rb_type(entry = rb_ary_entry(match, i)); i++) {
                            rargs[1] = 0; ///< Reset matcher count
        ...
        rb_raise(rb_eArgError, "Unexpected value type for tag `%s'", rb_obj_classname(name));
    }
...
static VALUE RubyConfigView(int argc, VALUE *argv, VALUE self) {
    int flags = 0;
        ...
        if (!(subtle->flags & SUB_SUBTLE_CHECK)) {
            ...
            if ((v = subViewNew(RSTRING_PTR(name), re))) {
                ...
                if (!NIL_P(icon)) {
                    ...
                    v->flags &= ~SUB_VIEW_ICON_ONLY;
                }
            }
        }
    } else {
        rb_raise(rb_eArgError, "Unexpected value type for sublet `%s'", rb_obj_classname(name));
    }
...
static VALUE RubyConfigScreen(VALUE self, VALUE screenid) {
    rb_need_block();
    ...
    if (FIXNUM_P(screenid)) {
        ...
        /* Eval virtuals */
        if (T_ARRAY == rb_type(value = rb_hash_lookup(params, CHAR2SYM("virtual")))) {
        ...
        rb_raise(rb_eArgError, "Unexpected value type for screen `%s'", rb_obj_classname(screenid));
    }
...
static VALUE RubyConfigSublet(VALUE self, VALUE sublet) {
    VALUE klass = Qnil, options = Qnil;
    ...
    if (T_SYMBOL == rb_type(sublet)) {
        ...
        rb_hash_aset(config_sublets, sublet, rb_obj_clone(rb_iv_get(options, "@params")));
    } else {
        rb_raise(rb_eArgError, "Unexpected value type for sublet `%s'", rb_obj_classname(sublet));
    }
...
static VALUE RubyConfigOn(int argc, VALUE *argv, VALUE self) {
    ...
    if (T_SYMBOL == rb_type(event)) {
        ...
        rb_raise(rb_eArgError, "Unexpected value type for on `%s'", rb_obj_classname(event));
    }
...
static VALUE RubySubletDispatcher(int argc, VALUE *argv, VALUE self) {
    ...
    if (rb_respond_to(self, rb_to_id(missing))) { ///< Intance methods
        ...
        if (index(name, '=')) {
            ...
            ret = rb_funcall(self, rb_intern("instance_variable_get"), 1, CHAR2SYM(buf));
        }
    }
...
static VALUE RubySubletConfig(VALUE self) {
    SubPanel *p = NULL;
    VALUE hash = Qnil;
...
static VALUE RubySubletConfigure(VALUE self, VALUE name) {
    rb_need_block();
    ...
    if (T_SYMBOL == rb_type(name)) {
        ...
        if (p) {
            ...
            rb_funcall(rb_singleton_class(p->sublet->instance), rb_intern("define_method"), 2,
                       CHAR2SYM("__configure"), proc);
        }
    } else {
        rb_raise(rb_eArgError, "Unexpected value type for configure `%s'",
...
static VALUE RubySubletHelper(VALUE self) {
    SubPanel *p = NULL;
...
static VALUE RubySubletOn(VALUE self, VALUE event) {
    /* Check value type */
            ...
            for (i = 0; LENGTH(methods) > i; i++) {
                if (methods[i].sym == event) {
                    ...
                    if (-1 == arity || (1 <= arity && methods[i].arity >= arity)) {
                        /* Add flags */
                        if (methods[i].flags & (SUB_PANEL_DOWN | SUB_PANEL_OVER | SUB_PANEL_OUT)) {
                        ...
                        rb_raise(rb_eArgError, "Wrong number of arguments (%d for %d)", arity,
                                 methods[i].arity);
                    }
                }
            }
...
static VALUE RubySubletGrab(VALUE self, VALUE name) {
    /* Check value type */
    if (T_SYMBOL == rb_type(name) && rb_block_given_p()) {
        ...
        rb_raise(rb_eArgError, "Unexpected value type for grab `%s'", rb_obj_classname(name));
    }
...
static VALUE RubySubletRender(VALUE self) {
    SubPanel *p = NULL;
...
static VALUE RubySubletNameReader(VALUE self) {
    SubPanel *p = NULL;
...
static VALUE RubySubletIntervalReader(VALUE self) {
    SubPanel *p = NULL;
...
static VALUE RubySubletIntervalWriter(VALUE self, VALUE value) {
    SubPanel *p = NULL;
    ...
    if (p) {
        if (FIXNUM_P(value)) {
            ...
            rb_raise(rb_eArgError, "Unexpected value type for interval `%s'",
                     rb_obj_classname(value));
        }
    }
...
static VALUE RubySubletDataReader(VALUE self) {
    int i;
    VALUE string = Qnil;
    SubPanel *p = NULL;
    ...
    if (p) {
        if (0 < p->sublet->text->nitems) {
            ...
            for (i = 0; i < p->sublet->text->nitems; i++) {
                ...
                if (Qnil == string) {
                    rb_str_new2(item->data.string);
                } else {
                    rb_str_cat(string, item->data.string, strlen(item->data.string));
                }
            }
        }
    }
...
static VALUE RubySubletDataWriter(VALUE self, VALUE value) {
    SubPanel *p = NULL;
    ...
    if (p) {
        ...
        if (T_STRING == rb_type(value)) {
            ...
            p->sublet->width =
                    subTextParse(p->sublet->text, subtle->styles.sublets.font, RSTRING_PTR(value)) +
                    STYLE_WIDTH((*s));
        } else {
            rb_raise(rb_eArgError, "Unexpected value type for data `%s'",
                     rb_obj_classname(value));
        }
    }
...
RubySubletGeometryReader(VALUE self) {
    ...
    if (p) {
        ...
        subtlext = rb_const_get(rb_mKernel, rb_intern("Subtlext"));
        klass = rb_const_get(subtlext, rb_intern("Geometry"));
        geometry = rb_funcall(klass, rb_intern("new"), 4, INT2FIX(geom.x), INT2FIX(geom.y),
...
RubySubletScreenReader(VALUE self) {
    ...
    Data_Get_Struct(self, SubPanel, p);
    if (p) {
        screen = RubySubtleToSubtlext(p->screen);
    }
...
static VALUE RubySubletStyleWriter(VALUE self, VALUE value) {
    SubPanel *p = NULL;
    ...
    if (p) {
        ...
        if (FIXNUM_P(value)) {
            ...
            rb_raise(rb_eArgError, "Unexpected value type for style `%s'",
                     rb_obj_classname(value));
        }
    }
...
static VALUE RubySubletShow(VALUE self) {
    SubPanel *p = NULL;
...
static VALUE RubySubletHide(VALUE self, VALUE value) {
    SubPanel *p = NULL;
...
static VALUE RubySubletWatch(VALUE self, VALUE value) {
    VALUE ret = Qfalse;
    SubPanel *p = NULL;

    Data_Get_Struct(self, SubPanel, p);
    if (p) {
        if (!(p->sublet->flags & (SUB_SUBLET_SOCKET | SUB_SUBLET_INOTIFY)) && RTEST(value)) {
            ...
            if (FIXNUM_P(value) || rb_respond_to(value, rb_intern("fileno"))) {
                ...
                if (FIXNUM_P(value)) {
                    p->sublet->watch = FIX2INT(value);
                } else {
                    p->sublet->watch = FIX2INT(rb_funcall(value, rb_intern("fileno"), 0, NULL));
                }

                XSaveContext(subtle->dpy, subtle->windows.support, p->sublet->watch, (void *) p);
                subEventWatchAdd(p->sublet->watch);

                /* Set nonblocking */
                if (-1 == (flags = fcntl(p->sublet->watch, F_GETFL, 0))) {
                    flags = 0;
                }
            ...
            else if (T_STRING == rb_type(value)) /// Inotify file
            {
                ...
                if (0 < (p->sublet->watch = inotify_add_watch(subtle->notify, buf, IN_MODIFY))) {
                    ...
                    subSubtleLogWarn("Cannot watch file `%s': %s\n", buf, strerror(errno));
                }
            }
...
static VALUE RubySubletUnwatch(VALUE self) {
    VALUE ret = Qfalse;
    SubPanel *p = NULL;
...
static VALUE RubySubletWarn(VALUE self, VALUE str) {
    SubPanel *p = NULL;
...
void subRubyLoadConfig(void) {
    ...
    if (Qfalse == RubyConfigLoadConfig(
                          config_instance,
...
void subRubyReloadConfig(void) {
    ...
    subtle->flags &= (SUB_SUBTLE_DEBUG | SUB_SUBTLE_EWMH | SUB_SUBTLE_RUN | SUB_SUBTLE_XINERAMA |
                      SUB_SUBTLE_XRANDR | SUB_SUBTLE_URGENT);
    ...
    for (i = 0; i < subtle->sublets->ndata; i++) {
        ...
        p->flags &= ~(SUB_PANEL_BOTTOM | SUB_PANEL_SPACER1 | SUB_PANEL_SPACER1 |
                      SUB_PANEL_SEPARATOR1 | SUB_PANEL_SEPARATOR2);
        p->screen = NULL;
    }
    ...
    for (i = 0; i < subtle->clients->ndata; i++) {
        ...
        c->gravityid = -1;
        c->flags = (c->flags & (SUB_TYPE_CLIENT | SUB_CLIENT_FOCUS | SUB_CLIENT_INPUT |
                                SUB_CLIENT_CLOSE)); ///< Reset flags
    ...
    } else if ((c = subClientNext(0, False))) {
        subClientFocus(c, True);
    }
...
void subRubyLoadSublet(const char *file) {
    int state = 0;
    SubPanel *p = NULL;
    VALUE rargs[3] = {Qnil};
    ...
    p->sublet->instance =
            Data_Wrap_Struct(rb_const_get(mod, rb_intern("Sublet")), NULL, NULL, (void *) p);
...
void subRubyUnloadSublet(SubPanel *p) {
    int i;
    ...
    for (i = 0; i < subtle->grabs->ndata; i++) {
        ...
        if (grab->flags & SUB_GRAB_PROC && RubyReceiver(p->sublet->instance, grab->data.num)) {
...
void subRubyLoadSublets(void) {
    ...
    if (!subtle->notify) {
        if (0 > (subtle->notify = inotify_init())) {
            ...
            fcntl(subtle->notify, F_SETFL, O_NONBLOCK);
        }
    }
#endif /* HAVE_SYS_INOTIFY_H */
    ...
    if (subtle->paths.sublets)
        ...
        if ((home = getenv("XDG_DATA_HOME"))) {
            len += snprintf(buf, sizeof(buf), "%s/%s/sublets", home, PKG_NAME);
        } else {
            len += snprintf(buf, sizeof(buf), "%s/.local/share/%s/sublets", getenv("HOME"),
                            PKG_NAME);
        }
    }
...
int subRubyCall(int type, unsigned long proc, void *data) {
    int state = 0;
    VALUE rargs[3] = {Qnil};
...
int subRubyRelease(unsigned long value) {
    int state = 0;
