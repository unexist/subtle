...
/* SubtleSignal {{{ */
static void SubtleSignal(int signum) {
    switch (signum) {
        case SIGCHLD:
            wait(NULL);
            break;
...
/* SubtleVersion {{{ */
static void SubtleVersion(void) {
    printf("%s %s - Copyright (c) 2005-present Christoph Kappel\n"
...
#ifdef DEBUG
/* SubtleLevel {{{ */
static int SubtleLevel(const char *str) {
    int level = 0;
...
XPointer *subSubtleFind(Window win, XContext id) {
    XPointer *data = NULL;
...
int main(int argc, char *argv[]) {
    ...
    char *display = NULL;
    struct sigaction sa;
    const struct option long_options[] = {{"config", required_argument, 0, 'c'},
