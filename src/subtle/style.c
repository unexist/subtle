...
/* StyleInheritSides {{{ */
static void StyleInheritSides(SubSides *s1, SubSides *s2, int merge) {
    if (-1 == s1->top || (merge && -1 != s2->top)) {
        s1->top = s2->top;
    }
    if (-1 == s1->right || (merge && -1 != s2->right)) {
        s1->right = s2->right;
    }
    if (-1 == s1->bottom || (merge && -1 != s2->bottom)) {
        s1->bottom = s2->bottom;
    }
    if (-1 == s1->left || (merge && -1 != s2->left)) {
        s1->left = s2->left;
    }
} /* }}} */
...
static void StyleInherit(SubStyle *s1, SubStyle *s2) {
    ...
    if (s1->font) {
        ...
        if (s1->separator) {
            s1->separator->width =
                    subSharedStringWidth(subtle->dpy, s1->font, s1->separator->string,
    ...
    if (s1->styles) {
        ...
        for (i = 0; i < s1->styles->ndata; i++) {
            ...
            if (subtle->styles.urgent == style || subtle->styles.visible == style) {
                continue;
            }
...
/* StyleFont {{{ */
static void StyleFont(SubStyle *s, const char *name) {
...
SubStyle *subStyleFind(SubStyle *s, char *name, int *idx) {
    SubStyle *found = NULL;
...
void subStyleReset(SubStyle *s, int val) {
    assert(s);
...
void subStyleMerge(SubStyle *s1, SubStyle *s2) {
    assert(s1 && s2);
