...
SubGravity *subGravityNew(const char *name, XRectangle *geom) {
    SubGravity *g = NULL;
    ...
    if (name) {
        g->quark = XrmStringToQuark(name); ///< Create hash
    }
...
void subGravityGeometry(SubGravity *g, XRectangle *bounds, XRectangle *geom) {
...
void subGravityKill(SubGravity *g) {
    assert(g);
...
int subGravityFind(const char *name, int quark) {
    int found = -1;
...
void subGravityPublish(void) {
    ...
    for (i = 0; i < subtle->gravities->ndata; i++) {
        ...
        /* Add gravity to list */
        snprintf(buf, sizeof(buf), "%dx%d+%d+%d#%s", g->geom.x, g->geom.y, g->geom.width,
                 g->geom.height, XrmQuarkToString(g->quark));
    ...
    subSharedPropertySetStrings(subtle->dpy, ROOT, subEwmhGet(SUB_EWMH_SUBTLE_GRAVITY_LIST),
                                gravities, subtle->gravities->ndata);
