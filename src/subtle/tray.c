...
SubTray *subTrayNew(Window win) {
    ...
    subEwmhMessage(t->win, SUB_EWMH_XEMBED, 0xFFFFFF, CurrentTime, XEMBED_EMBEDDED_NOTIFY, 0,
                   subtle->windows.tray, 0);
...
void subTrayConfigure(SubTray *t) {
    long supplied = 0;
...
void subTraySetState(SubTray *t) {
    long flags = 0;
    ...
    if ((flags = subEwmhGetXEmbedState(t->win))) {
        ...
        subEwmhMessage(t->win, SUB_EWMH_XEMBED, 0xFFFFFF, CurrentTime, opcode, 0, 0, 0);
    }
} /* }}} */
