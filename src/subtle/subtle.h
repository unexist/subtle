...
#define DEFAULT_LOGLEVEL \
    (SUB_LOG_WARN | SUB_LOG_ERROR | SUB_LOG_SUBLET | SUB_LOG_DEPRECATED) ///< Default loglevel

#define DEBUG_LOGLEVEL \
    (SUB_LOG_EVENTS | SUB_LOG_RUBY | SUB_LOG_XERROR | SUB_LOG_SUBTLE | \
     SUB_LOG_DEBUG) ///< Debug loglevel
...
#define STYLE_TOP(S) (S.border.top + S.padding.top + S.margin.top) ///< Get style top
#define STYLE_RIGHT(S) (S.border.right + S.padding.right + S.margin.right) ///< Get style right
#define STYLE_BOTTOM(S) (S.border.bottom + S.padding.bottom + S.margin.bottom) ///< Get style bottom
#define STYLE_LEFT(S) (S.border.left + S.padding.left + S.margin.left) ///< Get style left
...
#define DEAD(C) \
    if (!C || C->flags & SUB_CLIENT_DEAD) \
        return; ///< Check dead clients

#define MINMAX(Val, Min, Max) ((Val < Min) ? Min : ((Val > Max) ? Max : Val)) ///< Value min/max

#define XYINRECT(Wx, Wy, R) \
    (Wx >= R.x && Wx <= (R.x + R.width) && Wy >= R.y && \
     Wy <= (R.y + R.height)) ///< Whether x/y is in rect
...
#define VISIBLETAGS(C, Tags) \
    (C && (Tags & c->tags \
           || C->flags & (SUB_CLIENT_TYPE_DESKTOP | \
                       SUB_CLIENT_MODE_STICK))) ///< Whether client is visible on tags

#define ROOT DefaultRootWindow(subtle->dpy) ///< Root window
#define SCRN DefaultScreen(subtle->dpy) ///< Default screen

/* Logging macros */
#define subSubtleLogError(...) subSubtleLog(SUB_LOG_ERROR, __FILE__, __LINE__, __VA_ARGS__);
#define subSubtleLogSubletError(SUBLET, ...) \
    subSubtleLog(SUB_LOG_SUBLET, SUBLET, __LINE__, __VA_ARGS__);
#define subSubtleLogWarn(...) subSubtleLog(SUB_LOG_WARN, __FILE__, __LINE__, __VA_ARGS__);
#define subSubtleLogDeprecated(...) \
    subSubtleLog(SUB_LOG_DEPRECATED, __FILE__, __LINE__, __VA_ARGS__);

#ifdef DEBUG
#define subSubtleLogDebugEvents(...) subSubtleLog(SUB_LOG_EVENTS, __FILE__, __LINE__, __VA_ARGS__);
#define subSubtleLogDebugRuby(...) subSubtleLog(SUB_LOG_RUBY, __FILE__, __LINE__, __VA_ARGS__);
#define subSubtleLogDebugSubtlext(...) \
    subSubtleLog(SUB_LOG_SUBTLEXT, __FILE__, __LINE__, __VA_ARGS__);
#define subSubtleLogDebugSubtle(...) subSubtleLog(SUB_LOG_SUBTLE, __FILE__, __LINE__, __VA_ARGS__);
#define subSubtleLogDebug(...) subSubtleLog(SUB_LOG_DEBUG, __FILE__, __LINE__, __VA_ARGS__);
#else /* DEBUG */
...
#define GRABMASK (ButtonPressMask | ButtonReleaseMask | PointerMotionMask)
/* }}} */
...
/* Shortcuts */
#define TYPES_ALL \
    (SUB_CLIENT_TYPE_DESKTOP | SUB_CLIENT_TYPE_DOCK | SUB_CLIENT_TYPE_SPLASH \
     | SUB_CLIENT_TYPE_DIALOG) ///< All type flags

#define MODES_ALL \
    (SUB_CLIENT_MODE_FULL | SUB_CLIENT_MODE_FLOAT | SUB_CLIENT_MODE_STICK \
     | SUB_CLIENT_MODE_URGENT | SUB_CLIENT_MODE_RESIZE | SUB_CLIENT_MODE_ZAPHOD \
...
typedef struct subsubtle_t { /* {{{ */
    ...
    struct {
        struct substyle_t all, views, title, sublets, separator, clients, panel_top, panel_bot,
                tray; ///< Subtle base styles

        struct substyle_t *urgent, *occupied, *focus, *visible, *view_sep,
                *sublet_sep; ///< For faster access to sub-styles
...
void subEwmhSetWindows(Window win, SubEwmh e, Window *values, int size); ///< Set window properties
void subEwmhSetCardinals(Window win, SubEwmh e, long *values,
                         int size); ///< Set cardinal properties
...
int subEwmhMessage(Window win, SubEwmh e, long mask, long data0, long data1, long data2, long data3,
...
void subPanelAction(SubArray *panels, int type, int x, int y, int button,
                    int bottom); ///< Handle panel action
...
void subSubtleLog(int level, const char *file, int line, const char *format,
                  ...); ///< Print messages
...
void subTagMatcherAdd(SubTag *t, int type, char *pattern, int and); ///< Add a matcher
...
void subTextRender(SubText *t, SubFont *f, GC gc, Window win, int x, int y, long fg, long icon,
                   long bg); ///< Render text
...
void subViewFocus(SubView *v, int screenid, int swap, int focus); ///< Focus view
