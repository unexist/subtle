...
int DisplayClaim(void) {
    ...
    XSetSelectionOwner(subtle->dpy, session, subtle->windows.support, CurrentTime);
    ...
    if (XGetSelectionOwner(subtle->dpy, session) == subtle->windows.support) {
        if (owner) {
            ...
            for (i = 0; i < WAITTIME; i++) {
                if (XCheckWindowEvent(subtle->dpy, owner, StructureNotifyMask, &event)
...
/* DisplayStyleToColor {{{ */
static void DisplayStyleToColor(SubStyle *s, unsigned long *colors, int *pos) {
    colors[(*pos)++] = s->fg;
    colors[(*pos)++] = s->bg;
    colors[(*pos)++] = s->top;
    colors[(*pos)++] = s->right;
...
/* DisplayXError {{{ */
static int DisplayXError(Display *disp, XErrorEvent *ev) {
#ifdef DEBUG
    if (subtle->loglevel & SUB_LOG_XERROR) {
        if (42 != ev->request_code) { /* X_SetInputFocus */
            ...
            subSubtleLog(SUB_LOG_XERROR, __FILE__, __LINE__, "%s: win=%#lx, request=%d\n", error,
...
void subDisplayInit(const char *display) {
    ...
    subtle->windows.support = XCreateSimpleWindow(subtle->dpy, ROOT, -100, -100, 1, 1, 0, 0, 0);

    sattrs.override_redirect = True;
    sattrs.event_mask = PropertyChangeMask;
    XChangeWindowAttributes(subtle->dpy, subtle->windows.support, CWEventMask | CWOverrideRedirect,
                            &sattrs);
    ...
    mask = GCLineWidth | GCLineStyle | GCJoinStyle | GCCapStyle | GCFillStyle;
    ...
    /* Create tray window */
    subtle->windows.tray = XCreateSimpleWindow(subtle->dpy, ROOT, 0, 0, 1, 1, 0, 0, 0);

    sattrs.override_redirect = True;
    sattrs.event_mask = KeyPressMask | ButtonPressMask;
    XChangeWindowAttributes(subtle->dpy, subtle->windows.tray, CWOverrideRedirect | CWEventMask,
                            &sattrs);
...
void subDisplayConfigure(void) {
    ...
    /* Update windows */
    XSetWindowBackground(subtle->dpy, subtle->windows.tray, subtle->styles.tray.bg);
...
void subDisplayFinish(void) {
    ...
    if (subtle->dpy) {
        ...
        /* Free cursors */
        if (subtle->cursors.arrow) {
            XFreeCursor(subtle->dpy, subtle->cursors.arrow);
        }
        if (subtle->cursors.move) {
            XFreeCursor(subtle->dpy, subtle->cursors.move);
        }
        if (subtle->cursors.resize) {
            XFreeCursor(subtle->dpy, subtle->cursors.resize);
        }

        /* Free GCs */
        if (subtle->gcs.stipple) {
            XFreeGC(subtle->dpy, subtle->gcs.stipple);
        }
        if (subtle->gcs.invert) {
            XFreeGC(subtle->dpy, subtle->gcs.invert);
        }
        if (subtle->gcs.draw) {
            XFreeGC(subtle->dpy, subtle->gcs.draw);
        }
