...
/* ScreenPublish {{{ */
static void ScreenPublish(void) {
    int i;
    ...
    workareas = (long *) subSharedMemoryAlloc(4 * subtle->screens->ndata, sizeof(long));
    panels = (long *) subSharedMemoryAlloc(2 * subtle->screens->ndata, sizeof(long));
    ...
    subEwmhSetCardinals(ROOT, SUB_EWMH_NET_WORKAREA, workareas, 4 * subtle->screens->ndata);
    subEwmhSetCardinals(ROOT, SUB_EWMH_SUBTLE_SCREEN_PANELS, panels, 2 * subtle->screens->ndata);

    /* EWMH: Desktop viewport */
    viewports = (long *) subSharedMemoryAlloc(2 * subtle->screens->ndata,
                                              sizeof(long)); ///< Calloc inits with zero - great

    subEwmhSetCardinals(ROOT, SUB_EWMH_NET_DESKTOP_VIEWPORT, viewports, 2 * subtle->screens->ndata);
...
/* ScreenClear {{{ */
static void ScreenClear(SubScreen *s, SubStyle *style) {
    /* Clear pixmap */
    XSetForeground(subtle->dpy, subtle->gcs.draw, style->bg);
    XFillRectangle(subtle->dpy, s->drawable, subtle->gcs.draw, 0, 0, s->base.width, subtle->ph);
    ...
    if (s->flags & SUB_SCREEN_STIPPLE) {
        ...
        XChangeGC(subtle->dpy, subtle->gcs.stipple, GCForeground | GCStipple, &gvals);

        XFillRectangle(subtle->dpy, s->drawable, subtle->gcs.stipple, 0, 0, s->base.width,
                       subtle->ph);
    }
} /* }}} */
...
void subScreenInit(void) {
        ...
        if ((res = XRRGetScreenResourcesCurrent(subtle->dpy, ROOT))) {
            ...
            for (i = 0; i < res->ncrtc; i++) {
                if ((crtc = XRRGetCrtcInfo(subtle->dpy, res, res->crtcs[i]))) {
                    ...
                    if (None != crtc->mode
                        && (s = subScreenNew(crtc->x, crtc->y, crtc->width, crtc->height))) {
    ...
    if (subtle->flags & SUB_SUBTLE_XINERAMA && 0 == subtle->screens->ndata &&
        ...
        if ((info = XineramaQueryScreens(subtle->dpy, &n))) {
            for (i = 0; i < n; i++) {
                /* Create new screen */
                if ((s = subScreenNew(info[i].x_org, info[i].y_org, info[i].width,
                                      info[i].height)))
                {
                    subArrayPush(subtle->screens, (void *) s);
                }
...
SubScreen *subScreenNew(int x, int y, unsigned int width, unsigned int height) {
    SubScreen *s = NULL;
    ...
    sattrs.event_mask = ButtonPressMask | EnterWindowMask | LeaveWindowMask | ExposureMask;
    sattrs.override_redirect = True;
    sattrs.background_pixmap = ParentRelative;
    mask = CWEventMask | CWOverrideRedirect | CWBackPixmap;

    s->panel1 = XCreateWindow(subtle->dpy, ROOT, 0, 1, 1, 1, 0, CopyFromParent, InputOutput,
                              CopyFromParent, mask, &sattrs);
    s->panel2 = XCreateWindow(subtle->dpy, ROOT, 0, 0, 1, 1, 0, CopyFromParent, InputOutput,
                              CopyFromParent, mask, &sattrs);
...
SubScreen *subScreenFind(int x, int y, int *sid) {
    int i;
...
void subScreenConfigure(void) {
    ...
    if (0 < subtle->clients->ndata) {
        ...
        for (i = 0; i < subtle->clients->ndata; i++) {
            ...
            if (0 < visible) {
                ...
                /* EWMH: Desktop, screen */
                subEwmhSetCardinals(c->win, SUB_EWMH_NET_WM_DESKTOP, (long *) &viewid, 1);
                subEwmhSetCardinals(c->win, SUB_EWMH_SUBTLE_CLIENT_SCREEN, (long *) &screenid, 1);
    ...
    subEwmhSetCardinals(ROOT, SUB_EWMH_SUBTLE_VISIBLE_TAGS, (long *) &subtle->visible_tags, 1);
    subEwmhSetCardinals(ROOT, SUB_EWMH_SUBTLE_VISIBLE_VIEWS, (long *) &subtle->visible_views, 1);
        ...
        for (j = 0, npanel = 0, center = False; s->panels && j < s->panels->ndata; j++) {
            ...
            if (p->flags & SUB_PANEL_TRAY) {
                if (0 < subtle->trays->ndata) {
                    ...
                    XFillRectangle(subtle->dpy, subtle->windows.tray, subtle->gcs.draw, 0, 0,
    ...
    for (i = 0; i < subtle->screens->ndata; i++) {
        ...
        for (j = 0; s->panels && j < s->panels->ndata; j++) {
            ...
            if (panel != s->panel2 && p->flags & SUB_PANEL_BOTTOM) {
                XCopyArea(subtle->dpy, s->drawable, panel, subtle->gcs.draw, 0, 0, s->base.width,
                          subtle->ph, 0, 0);
...
void subScreenResize(void) {
    ...
    for (i = 0; i < subtle->screens->ndata; i++) {
        ...
        if (s->flags & SUB_SCREEN_PANEL1) {
            XMoveResizeWindow(subtle->dpy, s->panel1, s->base.x, s->base.y, s->base.width,
                              subtle->ph);
            ...
            XUnmapWindow(subtle->dpy, s->panel1);
        }
        ...
        if (s->flags & SUB_SCREEN_PANEL2) {
            ...
            XUnmapWindow(subtle->dpy, s->panel2);
        }

        /* Create/update drawable for double buffering */
        if (s->drawable) {
            XFreePixmap(subtle->dpy, s->drawable);
        }
...
void subScreenWarp(SubScreen *s) {
    assert(s);

    /* Move pointer to screen center */
    XWarpPointer(subtle->dpy, None, ROOT, 0, 0, s->geom.x, s->geom.y, s->geom.x + s->geom.width / 2,
                 s->geom.y + s->geom.height / 2);
...
void subScreenKill(SubScreen *s) {
    assert(s);

    if (s->panels) {
        subArrayKill(s->panels, True);
    }
...
void subScreenPublish(void) {
    ...
    views = (long *) subSharedMemoryAlloc(subtle->screens->ndata, sizeof(long));
    ...
    subEwmhSetCardinals(ROOT, SUB_EWMH_SUBTLE_SCREEN_VIEWS, views, subtle->screens->ndata);
