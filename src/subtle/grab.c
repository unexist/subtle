...
void subGrabInit(void) {
    XModifierKeymap *modmap = XGetModifierMapping(subtle->dpy);
    if (modmap && 0 < modmap->max_keypermod) {
        const int modmasks[] = {ShiftMask, LockMask, ControlMask, Mod1Mask,
...
SubGrab *subGrabNew(const char *keys, int *duplicate) {
    int mouse = False;
    ...
    /* Parse keys */
    if (NoSymbol != (sym = subSharedParseKey(subtle->dpy, keys, &code, &state, &mouse))) {
        ...
        subSubtleLogWarn("Cannot assign grab `%s'\n", keys);
    }
...
SubGrab *subGrabFind(int code, unsigned int state) {
    ...
    gptr = &g;
    ret = (SubGrab **) bsearch(&gptr, subtle->grabs->data, subtle->grabs->ndata, sizeof(SubGrab *),
                               subGrabCompare);
...
void subGrabSet(Window win, int mask) {
    if (win) {
        int i, j;
        const unsigned int states[] = {0, LockMask, numlockmask, numlockmask | LockMask};
        ...
        for (i = 0; i < subtle->grabs->ndata; i++) {
            ...
            if (!(g->flags & (SUB_GRAB_CHAIN_LINK | SUB_GRAB_CHAIN_END)) && g->flags & mask) {
                /* FIXME: Ugly key/state grabbing */
                for (j = 0; LENGTH(states) > j; j++) {
                    if (g->flags & SUB_GRAB_KEY) {
                        XGrabKey(subtle->dpy, g->code, g->state | states[j], ROOT, True,
                                 GrabModeAsync, GrabModeAsync);
                    } else if (g->flags & SUB_GRAB_MOUSE) {
                        XGrabButton(subtle->dpy, g->code - XK_Pointer_Button1, g->state | states[j],
                                    win, False, ButtonPressMask | ButtonReleaseMask, GrabModeSync,
...
int subGrabCompare(const void *a, const void *b) {
    int ret = 0;
