...
/* EventUntag {{{ */
static void EventUntag(SubClient *c, int id) {
    int i, tag;
...
/* EventFindSublet {{{ */
static SubPanel *EventFindSublet(int id) {
    int i = 0, j = 0, idx = 0;
    ...
    for (i = 0; i < subtle->screens->ndata; i++) {
        ...
        for (j = 0; j < s->panels->ndata; j++) {
            ...
            if (p->flags & SUB_PANEL_SUBLET && !(p->flags & SUB_PANEL_COPY) && idx++ == id) {
                return p;
            }
        }
    }
...
/* EventQueuePush {{{ */
static void EventQueuePush(XClientMessageEvent *ev, long type) {
    /* Since we are dealing with race conditions we need to cache
     * client messages when a client/view/tag isn't ready yet */
    queue = (XClientMessageEvent *) subSharedMemoryRealloc(
            queue, (nqueue + 1) * sizeof(XClientMessageEvent));
    queue[nqueue] = *ev;
...
/* EventQueuePop {{{ */
static void EventQueuePop(long value, long type) {
    /* Check queue */
    if (queue) {
        int i;
        ...
        for (i = 0; i < nqueue; i++) {
            ...
            if (queue[i].data.l[4] == type) {
                ...
                nqueue--;
                queue = (XClientMessageEvent *) subSharedMemoryRealloc(
                        queue, nqueue * sizeof(XClientMessageEvent));
                i--;
            }
        }
    }
...
/* EventMatch {{{ */
static int EventMatch(int type, XRectangle *origin, XRectangle *test) {
...
/* EventColormap {{{ */
static void EventColormap(XColormapEvent *ev) {
...
/* EventConfigure {{{ */
static void EventConfigure(XConfigureEvent *ev) {
    /* Ckeck window */
    if (ROOT == ev->window) {
        int sw = 0, sh = 0;
        ...
        if (subtle->width == sw && subtle->height == sh) {
            return;
        }
...
static void EventConfigureRequest(XConfigureRequestEvent *ev) {
    SubClient *c = NULL;
    ...
    if ((c = CLIENT(subSubtleFind(ev->window, CLIENTID)))) {
            ...
            || c->flags & (SUB_CLIENT_MODE_FLOAT | SUB_CLIENT_MODE_RESIZE)))
        {
            ...
            if (ev->value_mask & CWWidth) {
                c->geom.width = ev->width;
            }
            if (ev->value_mask & CWHeight) {
                c->geom.height = ev->height;
            }
...
/* EventCrossing {{{ */
static void EventCrossing(XCrossingEvent *ev) {
    SubClient *c = NULL;
    SubScreen *s = NULL;
    ...
    switch (ev->type) {
        ...
        case LeaveNotify:
            if ((s = SCREEN(subSubtleFind(ev->window, SCREENID)))) { ///< Screen panels
                subPanelAction(s->panels, SUB_PANEL_OUT, ev->x, ev->y, -1, s->panel2 == ev->window);
            }
    }
...
/* EventDestroy {{{ */
static void EventDestroy(XDestroyWindowEvent *ev) {
    SubClient *c = NULL;
    SubTray *t = NULL;
    ...
    if ((c = CLIENT(subSubtleFind(ev->window, CLIENTID)))) { ///< Client
        ...
        /* Update focus if necessary */
        if (focus && (c = subClientNext(0, False))) {
            subClientFocus(c, True);
        }
    } else {
        int i;
...
/* EventExpose {{{ */
static void EventExpose(XExposeEvent *ev) {
    if (0 == ev->count) {
        subScreenRender(); ///< Render once
    }
...
/* EventFocus {{{ */
static void EventFocus(XFocusChangeEvent *ev) {
    /* Check window has focus or focus is caused by grabs */
    if (ev->window == subtle->windows.focus[0] || NotifyGrab == ev->mode
        || NotifyUngrab == ev->mode)
    {
        return;
    }
...
/* EventGrab {{{ */
static void EventGrab(XEvent *ev) {
    int x = 0, y = 0;
    ...
    switch (ev->type) {
        case ButtonPress:
            if ((s = SCREEN(subSubtleFind(ev->xbutton.window, SCREENID)))) { ///< Screen panels
                subPanelAction(s->panels, SUB_PANEL_DOWN, ev->xbutton.x, ev->xbutton.y,
                               ev->xbutton.button, s->panel2 == ev->xbutton.window);
    ...
    if (g) {
        FLAGS flag =
                g->flags & ~(SUB_TYPE_GRAB | SUB_GRAB_KEY | SUB_GRAB_MOUSE | SUB_GRAB_CHAIN_START |
                             SUB_GRAB_CHAIN_LINK | SUB_GRAB_CHAIN_END); ///< Clear mask
        ...
        if (g->flags & SUB_GRAB_CHAIN_START && g->keys) {
            ...
            if (subtle->panels.keychain.keychain) {
                ...
                if (0 < subtle->panels.keychain.keychain->len) {
                    buf[pos++] = ' ';
                }
                ...
                snprintf(subtle->panels.keychain.keychain->keys +
                         ...
                         subtle->panels.keychain.keychain->len + len, "%s%s", buf, key);
            ...
            XGrabKey(subtle->dpy, AnyKey, AnyModifier, ROOT, True, GrabModeAsync, GrabModeAsync);
        ...
        switch (flag) {
            ...
            case SUB_GRAB_WINDOW_RESIZE: /* {{{ */
                ...
                if ((c = CLIENT(subSubtleFind(subtle->windows.focus[0], CLIENTID)))
                    ...
                    && !(SUB_GRAB_WINDOW_RESIZE == flag && c->flags & SUB_CLIENT_MODE_FIXED))
                {
                    ...
                    /* Translate flags */
                    if (SUB_GRAB_WINDOW_MOVE == flag) {
                        flag = SUB_DRAG_MOVE;
                    } else if (SUB_GRAB_WINDOW_RESIZE == flag) {
                        flag = SUB_DRAG_RESIZE;
                    }
            ...
            case SUB_GRAB_WINDOW_TOGGLE: /* {{{ */
                if ((c = CLIENT(subSubtleFind(subtle->windows.focus[0], CLIENTID)))) {
                    ...
                    if (VISIBLE(c) || SUB_CLIENT_MODE_STICK == g->data.num) {
                        subScreenConfigure();
            ...
            case SUB_GRAB_WINDOW_SELECT: { /* {{{ */
                ...
                if ((c = CLIENT(subSubtleFind(subtle->windows.focus[0], CLIENTID)))) {
                    ...
                    for (i = 0; i < subtle->clients->ndata; i++) {
                        ...
                        if (c != k
                            && (subtle->visible_tags & k->tags || k->flags & SUB_CLIENT_MODE_STICK))
                        {
                            /* Substract stack position index to get top window */
                            distance = EventMatch(g->data.num, &c->geom, &k->geom) - i;
            ...
            case SUB_GRAB_WINDOW_GRAVITY: /* {{{ */
                if ((c = CLIENT(subSubtleFind(subtle->windows.focus[0], CLIENTID))) &&
                    ...
                    int i, id = -1, cid = 0, fid = (int) g->data.string[0] - GRAVITYSTRLIMIT,
                           size = strlen(g->data.string);

                    /* Remove float/fullscreen mode */
                    if (c->flags & (SUB_CLIENT_MODE_FLOAT | SUB_CLIENT_MODE_FULL)) {
                        subClientToggle(
                                c, c->flags & (SUB_CLIENT_MODE_FLOAT | SUB_CLIENT_MODE_FULL), True);
                        subScreenUpdate();
                        subScreenRender();
                    ...
                    for (i = 0; -1 == id && i < size; i++) {
                        ...
                        if (c->gravityid == cid) {
                            ...
                            if (i == size - 1) {
                                ...
                                id = (int) g->data.string[i + 1] - GRAVITYSTRLIMIT;
                            }
                        }
                    }
                    if (-1 == id)
            ...
            case SUB_GRAB_VIEW_SWAP: /* {{{ */
                if (g->data.num < subtle->views->ndata) {
                    ...
                    if (subtle->flags & SUB_SUBTLE_SKIP_WARP
                        && (c = CLIENT(subSubtleFind(subtle->windows.focus[0], CLIENTID)))
                        && VISIBLE(c))
                    {
                        sid = c->screenid;
                    } else {
                        subScreenFind(x, y, &sid);
                    }
            ...
            case SUB_GRAB_VIEW_SELECT: /* {{{ */
            {
                ...
                /* Find screen: Prefer screen of current window */
                if (subtle->flags & SUB_SUBTLE_SKIP_WARP &&
                    (c = CLIENT(subSubtleFind(subtle->windows.focus[0], CLIENTID))) && VISIBLE(c)) {
            ...
            case SUB_GRAB_SUBTLE_RESTART: /* {{{ */
                ...
                break; /* }}} */
            default:
                subSubtleLogWarn("Cannot find grab action!\n");
        }
    }
} /* }}} */

/* EventMap {{{ */
static void EventMap(XMapEvent *ev) {
    SubTray *t = NULL;
...
/* EventMapping {{{ */
static void EventMapping(XMappingEvent *ev) {
...
/* EventMapRequest {{{ */
static void EventMapRequest(XMapRequestEvent *ev) {
    SubClient *c = NULL;
    ...
    if ((c = CLIENT(subSubtleFind(ev->window, CLIENTID)))) {
        ...
        /* Hook: Create */
        subHookCall((SUB_HOOK_TYPE_CLIENT | SUB_HOOK_ACTION_CREATE), (void *) c);
    }
...
/* EventMessage {{{ */
static void EventMessage(XClientMessageEvent *ev) {
    SubClient *c = NULL;
    SubTray *r = NULL;
        ...
        switch (subEwmhFind(ev->message_type)) {
            ...
            case SUB_EWMH_NET_CURRENT_DESKTOP: /* {{{ */
                ...
                if (0 <= ev->data.l[0] && ev->data.l[0] < subtle->views->ndata) {
                    ...
                    subViewFocus(subtle->views->data[ev->data.l[0]], sid, True, True);
                }
                break; /* }}} */
                ...
                if ((c = CLIENT(subSubtleFind(ev->data.l[0], CLIENTID)))) {
                    if (!(VISIBLE(c))) { ///< Client is on current view?
                        ...
                        for (i = 0; i < subtle->views->ndata; i++) {
                            ...
                            {
                                subViewFocus(VIEW(subtle->views->data[i]), c->screenid, False,
                                             True);
                                break;
                            }
                        }
                    ...
                    XSetInputFocus(subtle->dpy, r->win, RevertToPointerRoot, CurrentTime);
                }
                break; /* }}} */
            case SUB_EWMH_NET_RESTACK_WINDOW: /* {{{ */
                if ((c = CLIENT(subSubtleFind(ev->data.l[1], CLIENTID)))) {
                    subClientRestack(c, Above == ev->data.l[2] ? SUB_CLIENT_RESTACK_UP
                                                               : SUB_CLIENT_RESTACK_DOWN);
                }
                break; /* }}} */
            ...
            case SUB_EWMH_SUBTLE_CLIENT_TAGS: /* {{{ */
                ...
                if ((c = CLIENT(subSubtleFind(ev->data.l[0], CLIENTID)))) {
                    ...
                    if (subtle->windows.focus[0] == c->win && !VISIBLE(c)) {
                        if ((c = subClientNext(c->screenid, False))) {
                            subClientFocus(c, True);
                        }
                    }
            ...
            case SUB_EWMH_SUBTLE_CLIENT_RETAG: /* {{{ */
                if ((c = CLIENT(subSubtleFind(ev->data.l[0], CLIENTID)))) {
                    ...
                    if (VISIBLE(c) || flags & SUB_CLIENT_MODE_FULL) {
                        subScreenConfigure();
                        subScreenUpdate();
                        subScreenRender();
                    }
            ...
            case SUB_EWMH_SUBTLE_CLIENT_GRAVITY: /* {{{ */
                /* Check if client is ready yet otherwise queue it */
                if ((c = CLIENT(subSubtleFind(ev->data.l[0], CLIENTID)))) {
                    if ((g = GRAVITY(subArrayGet(subtle->gravities, (int) ev->data.l[1])))) {
            ...
            case SUB_EWMH_SUBTLE_CLIENT_FLAGS: /* {{{ */
                ...
                if ((c = CLIENT(subSubtleFind(ev->data.l[0], CLIENTID)))) {
                    ...
                    /* Translate flags */
                    if (ev->data.l[1] & SUB_EWMH_FULL) {
                        flags |= SUB_CLIENT_MODE_FULL;
                    }
                    if (ev->data.l[1] & SUB_EWMH_FLOAT) {
                        flags |= SUB_CLIENT_MODE_FLOAT;
                    }
                    if (ev->data.l[1] & SUB_EWMH_STICK) {
                        flags |= SUB_CLIENT_MODE_STICK;
                    }
                    if (ev->data.l[1] & SUB_EWMH_RESIZE) {
                        flags |= SUB_CLIENT_MODE_RESIZE;
                    }
                    if (ev->data.l[1] & SUB_EWMH_URGENT) {
                        flags |= SUB_CLIENT_MODE_URGENT;
                    }
                    if (ev->data.l[1] & SUB_EWMH_ZAPHOD) {
                        flags |= SUB_CLIENT_MODE_ZAPHOD;
                    }
                    if (ev->data.l[1] & SUB_EWMH_FIXED) {
                        flags |= SUB_CLIENT_MODE_FIXED;
                    }
                    if (ev->data.l[1] & SUB_EWMH_BORDERLESS) {
                    ...
                    if (VISIBLE(c) || flags & (SUB_CLIENT_MODE_FULL | SUB_CLIENT_MODE_URGENT)) {
                        subScreenConfigure();
                        subScreenUpdate();
                        subScreenRender();
                    }
            ...
            case SUB_EWMH_SUBTLE_GRAVITY_NEW: /* {{{ */
                if (ev->data.b) {
                    ...
                    sscanf(ev->data.b, "%hdx%hd+%hd+%hd#%s", &geom.x, &geom.y, &geom.width,
                           &geom.height, buf);
            ...
            case SUB_EWMH_SUBTLE_GRAVITY_FLAGS: /* {{{ */
                if ((g = GRAVITY(subArrayGet(subtle->gravities, (int) ev->data.l[0])))) {
                    ...
                    if (ev->data.l[1] & SUB_EWMH_HORZ) {
                        flags |= SUB_GRAVITY_HORZ;
                    }
                    if (ev->data.l[1] & SUB_EWMH_VERT) {
                        flags |= SUB_GRAVITY_VERT;
                    }
            ...
            case SUB_EWMH_SUBTLE_GRAVITY_KILL: /* {{{ */
                if ((g = GRAVITY(subArrayGet(subtle->gravities, (int) ev->data.l[0])))) {
                    ...
                    for (i = 0; i < subtle->clients->ndata; i++) {
                        if ((c = CLIENT(subtle->clients->data[i]))
                            && c->gravityid == ev->data.l[0])
                        {
            ...
            case SUB_EWMH_SUBTLE_SCREEN_JUMP: /* {{{ */
                if ((s = SCREEN(subArrayGet(subtle->screens, (int) ev->data.l[0])))) {
                    subScreenWarp(s);
                }
                break; /* }}} */
            ...
            case SUB_EWMH_SUBTLE_SUBLET_FLAGS: /* {{{ */
                if ((p = EventFindSublet((int) ev->data.l[0]))) {
                    /* Update visibility */
                    if (ev->data.l[1] & SUB_EWMH_VISIBLE && p->flags & SUB_PANEL_HIDDEN) {
                        p->flags &= ~SUB_PANEL_HIDDEN;
                        subScreenUpdate();
                        subScreenRender();
                    } else if (ev->data.l[1] & SUB_EWMH_HIDDEN && !(p->flags & SUB_PANEL_HIDDEN)) {
                        p->flags |= SUB_PANEL_HIDDEN;
                        subScreenUpdate();
                        subScreenRender();
                    }
            ...
            case SUB_EWMH_SUBTLE_TAG_NEW: /* {{{ */
                if (ev->data.b) {
                    ...
                    if ((t = subTagNew(ev->data.b, &duplicate)) && !duplicate) {
                        ...
                        /* Hook: Create */
                        subHookCall((SUB_HOOK_TYPE_TAG | SUB_HOOK_ACTION_CREATE), (void *) t);
                    }
                }
                break; /* }}} */
            ...
            case SUB_EWMH_SUBTLE_VIEW_NEW: /* {{{ */
                if (ev->data.b && (v = subViewNew(ev->data.b, NULL))) {
                    ...
                    /* Hook: Create */
                    subHookCall((SUB_HOOK_TYPE_VIEW | SUB_HOOK_ACTION_CREATE), (void *) v);
                }
                break; /* }}} */
            ...
            case SUB_EWMH_NET_SYSTEM_TRAY_OPCODE: /* {{{ */
                switch (ev->data.l[1]) {
                    ...
                    case XEMBED_REQUEST_FOCUS: /* {{{ */
                        subEwmhMessage(r->win, SUB_EWMH_XEMBED, 0xFFFFFF, CurrentTime,
    ...
    /* Messages for client windows {{{ */
        ...
        switch (subEwmhFind(ev->message_type)) {
            case SUB_EWMH_NET_WM_STATE: /* {{{ */
            {
                ...
                /* Update screen and focus */
                if (VISIBLE(c) || flags & SUB_CLIENT_MODE_STICK) {
                    subScreenConfigure();
...
/* EventProperty {{{ */
static void EventProperty(XPropertyEvent *ev) {
    SubClient *c = NULL;
    SubTray *t = NULL;
    int id = subEwmhFind(ev->atom);

    if (XA_WM_NAME == ev->atom) {
        id = SUB_EWMH_WM_NAME;
    }
    ...
    switch (id) {
        case SUB_EWMH_WM_NAME: /* {{{ */
            if ((c = CLIENT(subSubtleFind(ev->window, CLIENTID)))) {
                ...
                /* Hook: Rename */
                subHookCall((SUB_HOOK_TYPE_CLIENT | SUB_HOOK_ACTION_RENAME), (void *) c);
            }
            break; /* }}} */
        ...
        case SUB_EWMH_WM_HINTS: /* {{{ */
            if ((c = CLIENT(subSubtleFind(ev->window, CLIENTID)))) {
                ...
                if (VISIBLE(c) || flags & SUB_CLIENT_MODE_URGENT) {
                    subScreenUpdate();
                    subScreenRender();
                }
            }
...
/* EventSelection {{{ */
void EventSelection(XSelectionClearEvent *ev) {
    ...
    subSubtleLogDebugEvents("SelectionClear: win=%#lx, tray=%#lx, support=%#lx\n", ev->window,
                            subtle->windows.tray, subtle->windows.support);
} /* }}} */

/* EventUnmap {{{ */
static void EventUnmap(XUnmapEvent *ev) {
    SubClient *c = NULL;
    SubTray *t = NULL;
...
void subEventLoop(void) {
    ...
    /* Set tray selection */
    if (subtle->flags & SUB_SUBTLE_TRAY) {
        subTraySelect();
    }
    ...
    while (subtle && subtle->flags & SUB_SUBTLE_RUN) {
        ...
        if (0 < (nevents = poll(watches, nwatches, timeout * 1000))) {
                    ...
                    if (watches[i].fd == ConnectionNumber(subtle->dpy)) { ///< X events {{{
                        while (XPending(subtle->dpy)) { ///< X events
                            ...
                            switch (ev.type) {
                                ...
                                case EnterNotify:
                                case LeaveNotify:
                                    EventCrossing(&ev.xcrossing);
                                    break;
                                case DestroyNotify:
                                    EventDestroy(&ev.xdestroywindow);
                                    break;
                                case Expose:
                                    EventExpose(&ev.xexpose);
                                    break;
                                case FocusIn:
                                    EventFocus(&ev.xfocus);
                                    break;
                                case ButtonPress:
                                case KeyPress:
                                    EventGrab(&ev);
                                ...
                                case MapNotify:
                                    ...
                                    break;
                                case MappingNotify:
                                    EventMapping(&ev.xmapping);
                                    break;
                                case MapRequest:
                                    EventMapRequest(&ev.xmaprequest);
                                    break;
                                case ClientMessage:
                                    EventMessage(&ev.xclient);
                                    break;
                                case PropertyNotify:
                                    EventProperty(&ev.xproperty);
                                    break;
                                case SelectionClear:
                                    EventSelection(&ev.xselectionclear);
                                    break;
                                case UnmapNotify:
                        ...
                        if (0 < read(subtle->notify, buf, BUFLEN)) { ///< Inotify events
                            ...
                            if (event && IN_IGNORED != event->mask) {
                                if ((p = PANEL(
                                             subSubtleFind(subtle->windows.support, event->wd))))
                                {
                                    subRubyCall(SUB_CALL_WATCH, p->sublet->instance, NULL);
                                    subScreenUpdate();
                                    subScreenRender();
                                }
                            }
                        ...
                        if ((p = PANEL(subSubtleFind(subtle->windows.support, watches[i].fd)))) {
                            subRubyCall(SUB_CALL_WATCH, p->sublet->instance, NULL);
                            subScreenUpdate();
                            subScreenRender();
                        }
            ...
            if (0 < subtle->sublets->ndata) {
                ...
                while (p && p->sublet->flags & SUB_SUBLET_INTERVAL && p->sublet->time <= now) {
                    subRubyCall(SUB_CALL_RUN, p->sublet->instance, NULL);
        ...
        if (0 < subtle->sublets->ndata) {
            ...
            timeout = p->sublet->flags & SUB_SUBLET_INTERVAL ? p->sublet->time - now : 60;
            if (0 >= timeout) {
                timeout = 1; ///< Sanitize
            }
        } else timeout = 60;
