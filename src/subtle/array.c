...
void subArrayPush(SubArray *a, void *elem) {
    assert(a);
...
void subArrayInsert(SubArray *a, int pos, void *elem) {
    int i;
...
void subArrayRemove(SubArray *a, void *elem) {
    int i, idx;
...
void *subArrayGet(SubArray *a, int id) {
    assert(a);
...
int subArrayIndex(SubArray *a, void *elem) {
    int i;
...
void subArraySort(SubArray *a, int (*compar)(const void *a, const void *b)) {
    assert(a && compar);
...
void subArrayClear(SubArray *a, int clean) {
    if (a) {
        int i;
        SubClient *c = NULL;
        ...
        for (i = 0; clean && i < a->ndata; i++) {
            ...
            if ((c = CLIENT(a->data[i]))) {
                if (c->flags & SUB_TYPE_CLIENT) {
                    subClientKill(c);
                } else if (c->flags & SUB_TYPE_GRAB) {
                    subGrabKill(GRAB(c));
                } else if (c->flags & SUB_TYPE_GRAVITY) {
                    subGravityKill(GRAVITY(c));
                } else if (c->flags & SUB_TYPE_HOOK) {
                    subHookKill(HOOK(c));
                } else if (c->flags & SUB_TYPE_SCREEN) {
                    subScreenKill(SCREEN(c));
                } else if (c->flags & SUB_TYPE_STYLE) {
                    subStyleKill(STYLE(c));
                } else if (c->flags & SUB_TYPE_TAG) {
                    subTagKill(TAG(c));
                } else if (c->flags & SUB_TYPE_TRAY) {
                    subTrayKill(TRAY(c));
                } else if (c->flags & SUB_TYPE_VIEW) {
                    subViewKill(VIEW(c));
                } else if (c->flags & SUB_TYPE_PANEL) {
                    subPanelKill(PANEL(c));
                }
            }
        }
...
void subArrayKill(SubArray *a, int clean) {
    if (a) {
