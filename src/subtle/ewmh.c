...
void subEwmhInit(void) {
    ...
    XChangeProperty(subtle->dpy, ROOT, atoms[SUB_EWMH_NET_SUPPORTED], XA_ATOM, 32, PropModeReplace,
                    (unsigned char *) &atoms, SUB_EWMH_TOTAL);

    /* EWMH: Window manager information */
    subEwmhSetWindows(ROOT, SUB_EWMH_NET_SUPPORTING_WM_CHECK, &subtle->windows.support, 1);
    ...
    subEwmhSetString(subtle->windows.support, SUB_EWMH_SUBTLE_VERSION, PKG_VERSION);
...
long subEwmhGetWMState(Window win) {
    Atom type = None;
    int format = 0;
...
void subEwmhSetWindows(Window win, SubEwmh e, Window *values, int size) {
...
void subEwmhSetCardinals(Window win, SubEwmh e, long *values, int size) {
...
void subEwmhSetString(Window win, SubEwmh e, char *value) {
    XChangeProperty(subtle->dpy, win, atoms[e], atoms[SUB_EWMH_UTF8], 8, PropModeReplace,
...
void subEwmhSetWMState(Window win, long state) {
    CARD32 data[2];
    data[0] = state;
    ...
    XChangeProperty(subtle->dpy, win, atoms[SUB_EWMH_WM_STATE], atoms[SUB_EWMH_WM_STATE], 32,
...
void subEwmhTranslateWMState(Atom atom, int *flags) {
...
void subEwmhTranslateClientMode(int client_flags, int *flags) {
    /* Translate supported mode flags */
    if (client_flags & SUB_CLIENT_MODE_FULL) {
        *flags |= SUB_EWMH_FULL;
    }
    if (client_flags & SUB_CLIENT_MODE_FLOAT) {
        *flags |= SUB_EWMH_FLOAT;
    }
    if (client_flags & SUB_CLIENT_MODE_STICK) {
        *flags |= SUB_EWMH_STICK;
    }
    if (client_flags & SUB_CLIENT_MODE_RESIZE) {
        *flags |= SUB_EWMH_RESIZE;
    }
    if (client_flags & SUB_CLIENT_MODE_URGENT) {
        *flags |= SUB_EWMH_URGENT;
    }
    if (client_flags & SUB_CLIENT_MODE_ZAPHOD) {
        *flags |= SUB_EWMH_ZAPHOD;
    }
    if (client_flags & SUB_CLIENT_MODE_FIXED) {
        *flags |= SUB_EWMH_FIXED;
    }
    if (client_flags & SUB_CLIENT_MODE_BORDERLESS) {
