...
/* IconMark {{{ */
static void IconMark(SubtlextIcon *i) {
    if (i)
        rb_gc_mark(i->instance);
} /* }}} */

/* IconSweep {{{ */
static void IconSweep(SubtlextIcon *i) {
    if (i) {
...
/* IconEqual {{{ */
VALUE IconEqual(VALUE self, VALUE other) {
    int ret = False;
...
VALUE subextIconAlloc(VALUE self) {
    SubtlextIcon *i = NULL;
...
VALUE subextIconInit(int argc, VALUE *argv, VALUE self) {
    SubtlextIcon *i = NULL;
        ...
        if (T_STRING == rb_type(data[0])) { ///< Icon path
            ...
            if (-1 == access(buf, R_OK)) {
                ...
                if ((home = getenv("XDG_DATA_HOME"))) {
                    snprintf(buf, sizeof(buf), "%s/subtle/icons/%s", home, RSTRING_PTR(data[0]));
                } else {
                    snprintf(buf, sizeof(buf), "%s/.local/share/subtle/icons/%s", getenv("HOME"),
                             RSTRING_PTR(data[0]));
                }

                /* Check if icon exists */
                if (-1 == access(buf, R_OK)) {
                    rb_raise(rb_eStandardError, "Invalid icon `%s'", RSTRING_PTR(data[0]));
                }
            }

            /* Reading bitmap or pixmap icon file */
            if (BitmapSuccess != XReadBitmapFile(display, DefaultRootWindow(display), buf,
                                                 &i->width, &i->height, &i->pixmap, &hotx, &hoty))
            {
                ...
                if (XpmSuccess == XpmReadFileToPixmap(display, DefaultRootWindow(display), buf,
                                                      &i->pixmap, NULL, &attrs))
                {
                    i->flags |= ICON_PIXMAP;
                    i->width = attrs.width;
            ...
            i->pixmap =
                    XCreatePixmap(display, DefaultRootWindow(display), i->width, i->height, depth);
...
VALUE subextIconDrawPoint(int argc, VALUE *argv, VALUE self) {
    VALUE data[4] = {Qnil};
    ...
    if (FIXNUM_P(data[0]) && FIXNUM_P(data[1])) {
        ...
        if (i) {
            ...
            XDrawPoint(display, i->pixmap, i->gc, FIX2INT(data[0]), FIX2INT(data[1]));
...
VALUE subextIconDrawLine(int argc, VALUE *argv, VALUE self) {
    VALUE data[6] = {Qnil};

    rb_scan_args(argc, argv, "42", &data[0], &data[1], &data[2], &data[3], &data[4], &data[5]);
    ...
    if (FIXNUM_P(data[0]) && FIXNUM_P(data[1]) && FIXNUM_P(data[2]) && FIXNUM_P(data[3])) {
        ...
        if (i) {
            ...
            XDrawLine(display, i->pixmap, i->gc, FIX2INT(data[0]), FIX2INT(data[1]),
                      FIX2INT(data[2]), FIX2INT(data[3]));
...
VALUE subextIconDrawRect(int argc, VALUE *argv, VALUE self) {
    VALUE data[7] = {Qnil};

    rb_scan_args(argc, argv, "43", &data[0], &data[1], &data[2], &data[3], &data[4], &data[5],
                 &data[6]);
    ...
    if (FIXNUM_P(data[0]) && FIXNUM_P(data[1]) && FIXNUM_P(data[2]) && FIXNUM_P(data[3])) {
        ...
        if (i) {
            ...
            /* Draw rect */
            if (Qtrue == data[4]) {
                XFillRectangle(display, i->pixmap, i->gc, FIX2INT(data[0]), FIX2INT(data[1]),
                               FIX2INT(data[2]), FIX2INT(data[3]));
            } else {
                XDrawRectangle(display, i->pixmap, i->gc, FIX2INT(data[0]), FIX2INT(data[1]),
                               FIX2INT(data[2]), FIX2INT(data[3]));
            }
...
VALUE subextIconCopyArea(int argc, VALUE *argv, VALUE self) {
    VALUE data[7] = {Qnil};

    rb_scan_args(argc, argv, "16", &data[0], &data[1], &data[2], &data[3], &data[4], &data[5],
                 &data[6]);

    /* Check object type */
    if (rb_obj_is_instance_of(data[0], rb_const_get(mod, rb_intern("Icon")))) {
        ...
        if (src && dest) {
            ...
            /* Check args */
            if (!NIL_P(data[1])) {
                src_x = FIX2INT(data[1]);
            }
            if (!NIL_P(data[2])) {
                src_y = FIX2INT(data[2]);
            }
            if (!NIL_P(data[3])) {
                area_w = FIX2INT(data[3]);
            }
            if (!NIL_P(data[4])) {
                area_h = FIX2INT(data[4]);
            }
            if (!NIL_P(data[5])) {
                dest_x = FIX2INT(data[5]);
            }
            if (!NIL_P(data[6])) {
                dest_y = FIX2INT(data[6]);
            }
            ...
            if (area_w > dest_x + iwidth) {
                area_w = iwidth - dest_x;
            }
            if (area_h > dest_y + iheight) {
                area_h = iheight - dest_y;
            }

            if (0 > src_x || src_x > iwidth) {
                src_x = 0;
            }
            if (0 > src_y || src_y > iheight) {
                src_y = 0;
            }
            if (0 > dest_x || dest_x > iwidth) {
                dest_x = 0;
            }
            if (0 > dest_y || dest_y > iheight) {
                dest_y = 0;
            }
            ...
            if (src->flags & ICON_PIXMAP && dest->flags & ICON_PIXMAP) {
                XCopyPlane(display, src->pixmap, dest->pixmap, dest->gc, src_x, src_y, area_w,
                           area_h, dest_x, dest_y, 1);
            } else {
                XCopyArea(display, src->pixmap, dest->pixmap, dest->gc, src_x, src_y, area_w,
                          area_h, dest_x, dest_y);
            }
...
VALUE subextIconClear(int argc, VALUE *argv, VALUE self) {
    SubtlextIcon *i = NULL;
...
VALUE subextIconAskBitmap(VALUE self) {
    VALUE ret = Qfalse;
    SubtlextIcon *i = NULL;

    Data_Get_Struct(self, SubtlextIcon, i);
    if (i) {
        ret = (i->flags & ICON_BITMAP) ? Qtrue : Qfalse;
    }
...
VALUE subextIconToString(VALUE self) {
    VALUE ret = Qnil;
    SubtlextIcon *i = NULL;
    ...
    if (i) {
        ...
        snprintf(buf, sizeof(buf), "%s%c%ld%s", SEPARATOR, i->flags & ICON_PIXMAP ? '&' : '!',
                 i->pixmap, SEPARATOR);
        ret = rb_str_new2(buf);
    }
...
VALUE subextIconOperatorPlus(VALUE self, VALUE value) {
...
VALUE subextIconOperatorMult(VALUE self, VALUE value) {
    VALUE ret = Qnil;
...
VALUE subextIconEqual(VALUE self, VALUE other) { return IconEqual(self, other); } /* }}} */
...
VALUE subextIconEqualTyped(VALUE self, VALUE other) { return IconEqual(self, other); } /* }}} */
