...
/* SubtlextStringify {{{ */
static void SubtlextStringify(char *string) {
    assert(string);
...
/* SubtlextSweep {{{ */
static void SubtlextSweep(void) {
    if (display) {
...
static VALUE SubtlextPidReader(VALUE self) {
    Window win = None;
    VALUE pid = Qnil;
...
/* SubtlextFlags {{{ */
static int SubtlextFlags(VALUE key, VALUE value, VALUE data) {
    VALUE *rargs = (VALUE *) data;

    /* Set flags */
    if (CHAR2SYM("name") == key) {
        rargs[0] = SUB_MATCH_NAME;
    } else if (CHAR2SYM("instance") == key) {
        rargs[0] = SUB_MATCH_INSTANCE;
    } else if (CHAR2SYM("class") == key) {
        rargs[0] = SUB_MATCH_CLASS;
    } else if (CHAR2SYM("gravity") == key) {
        rargs[0] = SUB_MATCH_GRAVITY;
    } else if (CHAR2SYM("role") == key) {
        rargs[0] = SUB_MATCH_ROLE;
    } else if (CHAR2SYM("pid") == key) {
        rargs[0] = SUB_MATCH_PID;
    }
...
static VALUE SubtlextStyle(VALUE self, VALUE value) {
    char *prop = NULL;
    ...
    if (rb_obj_is_instance_of(self, rb_const_get(mod, rb_intern("View")))) {
        ...
        prop = "SUBTLE_SUBLET_STYLE";
    }

    /* Check value type */
    switch (rb_type(value)) {
        case T_SYMBOL:
            str = rb_sym_to_s(value);
            /* Falls-through */
        case T_STRING:
            snprintf(data.b, sizeof(data.b), "%d#%s", (int) FIX2INT(id), RSTRING_PTR(str));
...
static VALUE SubtlextHash(VALUE self) {
...
/* SubtlextXError {{{ */
static int SubtlextXError(Display *disp, XErrorEvent *ev) {
#ifdef DEBUG
...
/* SubtlextTagFind {{{ */
static int SubtlextTagFind(VALUE value) {
    int tags = 0;
    ...
    switch (rb_type(value)) {
        ...
        case T_STRING: {
            ...
            if (T_SYMBOL == rb_type(value)) {
                ...
                string = RSTRING_PTR(value);
            }

            /* Find tag and get id */
            if (-1 != (id = subextSubtlextFindString("SUBTLE_TAG_LIST", string, NULL, flags))) {
                tags |= (1L << (id + 1));
            }
        } break;
        case T_OBJECT:
...
/* SubtlextTag {{{ */
static VALUE SubtlextTag(VALUE self, VALUE value, int action) {
    SubMessageData data = {{0, 0, 0, 0, 0}};
...
static VALUE SubtlextTagWriter(VALUE self, VALUE value) {
...
static VALUE SubtlextTagReader(VALUE self) {
    char **tags = NULL;
    ...
    /* Check results */
    if ((tags = subSharedPropertyGetStrings(
                 display, ROOT, XInternAtom(display, "SUBTLE_TAG_LIST", False), &ntags))) {
        for (i = 0; i < ntags; i++) {
...
static VALUE SubtlextTagAdd(VALUE self, VALUE value) {
...
static VALUE SubtlextTagDel(VALUE self, VALUE value) {
    return SubtlextTag(self, value, -1);
} /* }}} */

/* SubtlextTagAsk {{{ */
static VALUE SubtlextTagAsk(VALUE self, VALUE value) {
    ...
    switch (rb_type(value)) {
        case T_STRING:
            sym = CHAR2SYM(RSTRING_PTR(value));
            break;
        case T_SYMBOL:
            /* Falls-through */
        case T_OBJECT:
    ...
    if (RTEST(tag = subextTagSingFirst(Qnil, sym))) {
        ...
        if (FIX2INT(tags) & (1L << (FIX2INT(id) + 1))) {
            ret = Qtrue;
        }
    }
...
/* SubtlextTagReload {{{ */
static VALUE SubtlextTagReload(VALUE self) {
    VALUE win = Qnil;
    SubMessageData data = {{0, 0, 0, 0, 0}};
...
static VALUE SubtlextSendButton(int argc, VALUE *argv, VALUE self) {
    Window subwin = None;
    XEvent event = {0};
    ...
    XTranslateCoordinates(display, event.xcrossing.window, event.xcrossing.root, event.xcrossing.x,
                          event.xcrossing.y, &event.xcrossing.x_root, &event.xcrossing.y_root,
                          &subwin);
...
static void SubtlextSendModifier(unsigned long state, int press) {
    /* Send modifier press/release events */
    if (state & ShiftMask) { ///< Shift key
        XTestFakeKeyEvent(display, XKeysymToKeycode(display, XK_Shift_L), press, CurrentTime);
    } else if (state & ControlMask) { ///< Ctrl key
        XTestFakeKeyEvent(display, XKeysymToKeycode(display, XK_Control_L), press, CurrentTime);
    } else if (state & Mod1Mask) { ///< Alt key
        XTestFakeKeyEvent(display, XKeysymToKeycode(display, XK_Alt_L), press, CurrentTime);
    } else if (state & Mod3Mask) { /// Mod key<
        XTestFakeKeyEvent(display, XKeysymToKeycode(display, XK_Meta_L), press, CurrentTime);
    } else if (state & Mod4Mask) { ///< Super key
        XTestFakeKeyEvent(display, XKeysymToKeycode(display, XK_Super_L), press, CurrentTime);
    } else if (state & Mod5Mask) { ///< Alt Gr key
        XTestFakeKeyEvent(display, XKeysymToKeycode(display, XK_ISO_Level3_Shift), press,
                          CurrentTime);
    }
} /* }}} */
#endif /* HAVE_X11_EXTENSIONS_XTEST_H */
...
static VALUE SubtlextSendKey(int argc, VALUE *argv, VALUE self) {
    ...
    if (T_STRING == rb_type(keys)) {
        ...
        /* Translate window x/y to root x/y */
        XTranslateCoordinates(display, event.xcrossing.window, event.xcrossing.root,
                              event.xcrossing.x, event.xcrossing.y, &event.xcrossing.x_root,
                              &event.xcrossing.y_root, &subwin);
        ...
        while (tok) {
            /* Parse key chain */
            if (NoSymbol == (sym = subSharedParseKey(display, tok, &code, &state, &mouse))) {
        ...
        rb_raise(rb_eArgError, "Unexpected value-type `%s'", rb_obj_classname(keys));
    }
...
static VALUE SubtlextFocus(VALUE self) {
    VALUE win = Qnil;
    SubMessageData data = {{0, 0, 0, 0, 0}};
...
static VALUE SubtlextAskFocus(VALUE self) {
    ...
    if ((focus = (unsigned long *) subSharedPropertyGet(
                 display, ROOT, XA_WINDOW, XInternAtom(display, "_NET_ACTIVE_WINDOW", False),
                 NULL)))
    {
        if (*focus == NUM2LONG(win)) {
            ret = Qtrue;
        }
...
static VALUE SubtlextPropReader(VALUE self, VALUE key) {
    char *prop = NULL;
    VALUE ret = Qnil;
    ...
    switch (rb_type(key)) {
        case T_STRING:
            prop = RSTRING_PTR(key);
            break;
        case T_SYMBOL:
            prop = (char *) SYM2CHAR(key);
            break;
        default:
            rb_raise(rb_eArgError, "Unexpected key value type `%s'", rb_obj_classname(key));
    ...
    if (prop) {
        ...
        if (rb_obj_is_instance_of(self, rb_const_get(mod, rb_intern("View")))) {
            ...
            snprintf(propname, sizeof(propname), "SUBTLE_PROPERTY_%s_%s", RSTRING_PTR(val), name);
        } else { ///< Client
        ...
        /* Get actual property */
        if ((result = subSharedPropertyGet(display, win, XInternAtom(display, "UTF8_STRING", False),
...
static VALUE SubtlextPropWriter(VALUE self, VALUE key, VALUE value) {
    ...
    switch (rb_type(value)) {
        ...
        case T_NIL:
            ...
            break;
        default:
            rb_raise(rb_eArgError, "Unexpected value value-type `%s'", rb_obj_classname(value));
    }
...
/* SubtlextEqual {{{ */
static VALUE SubtlextEqual(VALUE self, VALUE other, const char *attr, int check_type) {
    int ret = False;
...
/* SubtlextSpaceship {{{ */
static VALUE SubtlextSpaceship(VALUE self, VALUE other, const char *attr) {
    VALUE val1 = Qnil, val2 = Qnil;
...
static VALUE SubtlextEqualId(VALUE self, VALUE other) {
...
static VALUE SubtlextEqualWindow(VALUE self, VALUE other) {
...
static VALUE SubtlextEqualTypedId(VALUE self, VALUE other) {
...
static VALUE SubtlextEqualTypedWindow(VALUE self, VALUE other) {
...
static VALUE SubtlextEqualSpaceWindow(VALUE self, VALUE other) {
...
static VALUE SubtlextEqualSpacePixel(VALUE self, VALUE other) {
...
static VALUE SubtlextEqualSpacePixmap(VALUE self, VALUE other) {
...
static VALUE SubtlextEqualSpaceId(VALUE self, VALUE other) {
    return SubtlextSpaceship(self, other, "@id");
} /* }}} */

/* SubtlextWindowMatch {{{ */
static int SubtlextWindowMatch(Window win, regex_t *preg, const char *source, char **name,
                               int flags) {
    int ret = False;
    ...
    if (!ret && flags & (SUB_MATCH_INSTANCE | SUB_MATCH_CLASS)) {
        ...
        if (wminstance && flags & SUB_MATCH_INSTANCE) {
            ret = (flags & SUB_MATCH_EXACT ? 0 == strcmp(source, wminstance)
                                           : subSharedRegexMatch(preg, wminstance));
        }

        /* Check class */
        if (!ret && wmclass && flags & SUB_MATCH_CLASS) {
            ret = (flags & SUB_MATCH_EXACT ? 0 == strcmp(source, wmclass)
    ...
    if (!ret && flags & SUB_MATCH_GRAVITY) {
        ...
        gravities = subSharedPropertyGetStrings(
                display, ROOT, XInternAtom(display, "SUBTLE_GRAVITY_LIST", False), &ngravities);
        gravity = (int *) subSharedPropertyGet(display, win, XA_CARDINAL,
        ...
        if (gravities && gravity && 0 <= *gravity && *gravity < ngravities) {
            ret = (flags & SUB_MATCH_EXACT ? 0 == strcmp(source, gravities[*gravity])
    ...
    /* Copy instance name */
    if (ret && name) {
        *name = (char *) subSharedMemoryAlloc(strlen(wminstance) + 1, sizeof(char));
...
void subextSubtlextConnect(char *display_string) {
    /* Open display */
    if (!display) {
...
void subextSubtlextBacktrace(void) {
    VALUE lasterr = Qnil;
...
VALUE subextSubtlextConcat(VALUE str1, VALUE str2) {
    VALUE ret = Qnil;
    ...
    if (RTEST(str1) && RTEST(str2) && T_STRING == rb_type(str1)) {
        ...
        rb_raise(rb_eArgError, "Unexpected value type");
    }
...
VALUE subextSubtlextParse(VALUE value, char *buf, int len, int *flags) {
    VALUE ret = Qnil;
...
VALUE subextSubtlextOneOrMany(VALUE value, VALUE prev) {
    VALUE ret = Qnil;
    ...
    switch (rb_type(prev)) {
        ...
        default:
            rb_raise(rb_eArgError, "Unexpected value-type `%s'", rb_obj_classname(prev));
    }
...
VALUE subextSubtlextManyToOne(VALUE value) {
    VALUE ret = Qnil;
...
Window *subextSubtlextWindowList(char *prop_name, int *size) {
    Window *wins = NULL;
    unsigned long len = 0;
    ...
    /* Get property list */
    if ((wins = (Window *) subSharedPropertyGet(display, ROOT, XA_WINDOW,
                                                XInternAtom(display, prop_name, False), &len))) {
        if (size) {
            *size = len;
        }
    } else if (size) {
...
int subextSubtlextFindString(char *prop_name, char *source, char **name, int flags) {
    int ret = -1, size = 0;
    char **strings = NULL;
    regex_t *preg = NULL;
    ...
    strings = subSharedPropertyGetStrings(display, ROOT, XInternAtom(display, prop_name, False),
                                          &size);
    ...
    if (preg && strings) {
        ...
        for (i = 0; i < size; i++) {
            if (selid == i
                ...
                || (preg && !(flags & SUB_MATCH_EXACT) && subSharedRegexMatch(preg, strings[i])))))
            {
                if (name) {
                    *name = strdup(strings[i]);
                }
...
VALUE subextSubtlextFindObjects(char *prop_name, char *class_name, char *source, int flags, int first) {
    int i, nstrings = 0;
    char **strings = NULL;
    ...
    /* Check results */
    if ((strings = subSharedPropertyGetStrings(
                 display, ROOT, XInternAtom(display, prop_name, False), &nstrings))) {
        int selid = -1;
        ...
        for (i = 0; i < nstrings; i++) {
            ...
            {
                /* Create new object */
                if (RTEST((obj = rb_funcall(klass, meth_new, 1, rb_str_new2(strings[i]))))) {
                    ...
                    if (first) {
                        ...
                        ret = subextSubtlextOneOrMany(obj, ret);
                    }
                }
            }
        }
        ...
        rb_raise(rb_eStandardError, "Unknown property list `%s'", prop_name);
    }
...
VALUE subextSubtlextFindWindows(char *prop_name, char *class_name, char *source, int flags, int first) {
    int i, size = 0;
    Window *wins = NULL;
    VALUE ret = first ? Qnil : rb_ary_new();
    ...
    if ((wins = subextSubtlextWindowList(prop_name, &size))) {
        ...
        if ('#' == source[0]) {
            selwin = subextSubtleSingSelect(Qnil);
        }
        ...
        for (i = 0; i < size; i++) {
            if (selid == i || selid == wins[i] || selwin == wins[i]
                || (-1 == selid && SubtlextWindowMatch(wins[i], preg, source, NULL, flags)))
            {
                /* Create new obj */
                if (RTEST((obj = rb_funcall(klass, meth_new, 1, LONG2NUM(wins[i]))))) {
                    /* Call update method of object */
                    rb_funcall(obj, meth_update, 0, Qnil);
...
VALUE subextSubtlextFindObjectsGeometry(char *prop_name, char *class_name, char *source, int flags,
                                  int first)
{
    int nstrings = 0;
    char **strings = NULL;
    ...
    /* Get string list */
    if ((strings =
                 subSharedPropertyGetStrings(display, DefaultRootWindow(display),
                                             XInternAtom(display, prop_name, False), &nstrings)))
    {
        ...
        /* Create object list */
        for (i = 0; i < nstrings; i++) {
            sscanf(strings[i], "%hdx%hd+%hd+%hd#%s", &geometry.x, &geometry.y, &geometry.width,
                   &geometry.height, buf);

            /* Check if string matches */
            if (!source
                || (source && (selid == i || (-1 == selid
